...
plt.shap.summ <- xgb.ggplot.shap.summary(prepped$Full[[1]], contr, model = xgb_models$Full, top_n = 20) + 
...
plot_reliability_diagram <- function(data_frame, pred_col_name, true_label_col_name, title_suffix, line_color = "blue") {
  ...
  p <- ggplot(plot_data, aes(x = bin_midpoint, y = observed_proportion)) +
...
plot_reliability_diagram(office_test, "Pred.Full", "DM", "Original Uncalibrated (Pred.Full)", "darkgreen")
brier_score_original_full <- calculate_brier_score(office_test$Pred.Full, office_test$DM)
...
for (col_name in prediction_columns) {
  ...
  calibration_data_sorted <- office_test %>%
  ...
  plot_reliability_diagram(office_test, calibrated_col_name, "DM",
                           paste0("Isotonic Regression (", col_name, ")"), "darkblue")

  # Calculate Brier Score for Isotonic Regression predictions
  brier_score_isotonic <- calculate_brier_score(office_test[[calibrated_col_name]], office_test$DM)
  cat("Brier Score (Isotonic Regression for ", col_name, "):", round(brier_score_isotonic, 4), "\n")

  # Add to summary table
  brier_scores_summary <- rbind(brier_scores_summary,
                                data.frame(Prediction_Type = paste0(col_name, "_Isotonic"),
                                           Brier_Score = brier_score_isotonic))
}
...
platt_calibration_data <- office_test # Use office_test for calibration (for demonstration)
...
office_test$Pred.Full.Platt <- predict(platt_model_full, newdata = office_test, type = "response")

plot_reliability_diagram(office_test, "Pred.Full.Platt", "DM", "Platt Scaled (Pred.Full)", "purple")
brier_score_platt_full <- calculate_brier_score(office_test$Pred.Full.Platt, office_test$DM)
...
```{r}
library(probably)
office_test %>% mutate(DM=DM==TRUE) %>% cal_plot_breaks(DM,Pred.Full) %>% ggsave(filename="~/Dropbox/NHANES manuscript/figures/Binary-Calibration.bmp")
office_test %>% mutate(DM=DM==TRUE) %>% cal_plot_breaks(DM,Pred.SOC)
office_test %>% mutate(DM=DM==TRUE) %>% cal_plot_breaks(DM,Pred.SOC.Fast)
...
office_test %>% ggplot()+ geom_point(aes(x=a1c,y=Pred.SOC,color=fpg)) + facet_wrap(~DM,scales="free_x")
...
ot2 <- ot %>% mutate(quantile=cut_number(a1c,9))
...
roc_results %>% filter(Dx=="DM") %>% 
  ...
  ggplot() + 
  geom_point(data=roc_results.2 %>% filter(Dx=="DM"),aes(x=FPR,TPR,color=Type))+
  ...
  scale_color_manual(values=c("A1c"="darkgreen","FPG"="green","FPG-A1c"="cyan","NonFasting"="magenta","Full"="orange","Min"="red","SOC"="brown","SOC_Fast"="black","Test All (B)"="black","Test None (B)"="black")) +
    scale_linetype_manual(values=c("A1c"="longdash","FPG"="solid","FPG-A1c"="solid","NonFasting"="longdash","Full"="solid","Min"="solid","SOC"="longdash","SOC_Fast"="solid","Test All (B)"="dotted","Test None (B)"="dotted")) 
 #guides(
  #  shape = guide_legend(override.aes = list(linetype = c(1,2)))) # Remove the separate shape legend
...
```{r}
plt.roc.results.nonfast.dm <- 
roc_results %>% filter(Type=="A1c" | Type =="NonFasting" | Type=="SOC") %>% filter(Dx=="DM") %>% ggplot() + 
  geom_point(data=roc_results.2,aes(x=FPR,TPR,color=Type,shape=Dx,linetype=Dx))+
  geom_line(aes(x=FPR,y=TPR,color=Type,linetype=Dx,shape=Dx)) +
  labs(color="Model",linetype="Diagnosis",shape="Diagnosis") + 
  xlab('False Positive Rate') + 
  ...
  scale_linetype_discrete(labels=c("DM","Pre-DM")) + 
    ggtitle("Non-Fasting Models"); print(plt.roc.results.nonfast.dm) 
  

plt.roc.results.fast.dm <- roc_results %>% filter(Type=="FPG" | Type=="A1c" | Type=="FPG-A1c" | Type =="SOC_Fast" | Type=="Min") %>% filter(Dx=="DM") %>% ggplot() +
  geom_point(data=roc_results.2,aes(x=FPR,TPR,color=Type,shape=Dx,linetype=Dx))+
  geom_line(aes(x=FPR,y=TPR,color=Type,linetype=Dx,shape=Dx)) +
  labs(color="Model",linetype="Diagnosis",shape="Diagnosis") +
  xlab('False Positive Rate') +
  ...
  ggtitle("Fasting Models"); print(plt.roc.results.fast.dm)#, # Remove the separate shape legend
...
grid.arrange(nrow=3,heights=c(4,0.7,1.2),
             ...
             )) %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/ROC-DCA-figure-AB.bmp")
