...
hr.comp <- round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3+ RIDAGEYR + RIAGENDR + RIDRETH1, design=design_pred)[[1]][2]),2)
p.comp <- 0.90
```
...
plt.km.05.16.actual <-ggsurvplot_df(
  ...
  scale_color_manual(values = c("No DM" = "#1F78B4",
                                ...
                                "DM: OGTT only" = "#1B9E77")) +
  #add_legend_title("Diabetes\nDiagnosis") + 
  annotate("text",x=20,y=0.8,label=paste0("HR=",round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1, 
                                                                   design=design_pred)[[1]][1]),1),"\n",
                                          "p<0.00003"),
           color="#D81B60") + 
  annotate("text",x=20,y=0.6,label=paste0("HR=",round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1,
...
hr.a1c <-round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1, design=design_pred)[[1]][1]),2) 
hr.ogtt <-round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1, design=design_pred)[[1]][2]),2) 
hr.1 <- hr.a1c
hr.2 <- hr.ogtt
hr.3 <- hr.comp
p.3 <- p.comp
...
ggsave(plot=plt.km.05.16.actual + 
               ...
               theme.1 + coord_cartesian(xlim = c(0, 200)),filename="figures/KM-2005-2016.bmp",width=7,height=5,units=c("in"))
