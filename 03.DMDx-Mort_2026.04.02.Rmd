...
files.mort <- c("MORTALITY-DATA/NHANES_2005_2006_MORT_2019_PUBLIC.dat",
...
```{r}
df.mort %>% mutate(BMI= Weight/Height.Stand/Height.Stand*100*100) %>% select(SEQN,a1c,fpg,two,BMI, diabetes,ucod_leading,eligstat,mortstat,permth_int,permth_exm) %>% drop_na(two,a1c,fpg) %>% drop_na(permth_int) %>% mutate(Kind=1*(a1c>=6.5) + 2*(fpg>=126) + 4*(two>=200)) %>% unique() %>% head() %>% dput()
```



```{r}
df.mort <- df.mort %>% mutate(BMI= Weight/Height.Stand/Height.Stand*100*100)  %>% select(SEQN,Year,a1c,fpg,two,BMI,diabetes,ucod_leading,eligstat,mortstat,permth_int,permth_exm,sample.ogtt.weight) %>% drop_na(two,a1c,fpg,sample.ogtt.weight) %>%  mutate(Kind=1*(a1c>=6.5) + 2*(fpg>=126) + 4*(two>=200)) %>% unique()
```
...
hr.comp <- round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3, design=design_pred)[[1]][2]),2)
p.comp <- 0.04
```
...
# Log-rank test
#svylogrank(Surv(permth_int, mortstat) ~ Kind3 + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design = design_pred)#[[2]]

svycoxph(Surv(permth_int,mortstat) ~ Kind3+ RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)
exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3+ RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[1]][1])
exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3+ RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[1]][2])
hr.comp <- round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3+ RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[1]][2]),2)
p.comp <- 0.90

tab.comp <-summary(svycoxph(Surv(permth_int,mortstat) ~ Kind3  + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred))
hr.comp <- tab.comp$coefficients["Kind3OGTT only","exp(coef)"]
pval.comp <- tab.comp$coefficients["Kind3OGTT only","Pr(>|z|)"]
...
exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[1]][1])
exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[1]][2])
svycoxph(Surv(permth_int,mortstat) ~ Kind3 + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[6]][1]
svycoxph(Surv(permth_int,mortstat) ~ Kind3 + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[6]][2]
tab1<-summary(svycoxph(Surv(permth_int,mortstat) ~ Kind3  + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred))
```


```{r With No DM as a Refernece - with adjustments}
tab1$coefficients["Kind3A1c/FPG DMDx","Pr(>|z|)"]
tab1$coefficients["Kind3OGTT only","Pr(>|z|)"]
...
plt.km.05.16.actual <-ggsurvplot_df(
  ...
  scale_color_manual(values = c("No DM" = "#1F78B4",
                                ...
                                "DM: OGTT only" = "#1B9E77")) +
  #add_legend_title("Diabetes\nDiagnosis") + 
  annotate("text",x=20,y=0.8,label=paste0("HR=",round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, 
                                                                   design=design_pred)[[1]][1]),1),"\n",
                                          "p<0.00003"),
           color="#D81B60") + 
  annotate("text",x=20,y=0.6,label=paste0("HR=",round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1 + BMI,
                                                                  design=design_pred)[[1]][2]),1),"\n","p<0.0004"),color="#1B9E77");plot(plt.km.05.16.actual)
...
#hr.a1c <-round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[1]][1]),2) 
#hr.ogtt <-round(exp(svycoxph(Surv(permth_int,mortstat) ~ Kind3 +  RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred)[[1]][2]),2) 

tab1<-summary(svycoxph(Surv(permth_int,mortstat) ~ Kind3  + RIDAGEYR + RIAGENDR + RIDRETH1 + BMI, design=design_pred))
hr.a1c <- tab1$coefficients["Kind3A1c/FPG DMDx","exp(coef)"] %>% round(.,1)
hr.ogtt <- tab1$coefficients["Kind3OGTT only","exp(coef)"] %>% round(.,1)

pval.1 <- tab1$coefficients["Kind3A1c/FPG DMDx","Pr(>|z|)"] %>% round(5) 
pval.2 <- tab1$coefficients["Kind3OGTT only","Pr(>|z|)"] %>% round(3)

hr.1 <- hr.a1c
hr.2 <- hr.ogtt

hr.3 <- hr.comp %>% round(.,1)
pval.3 <- pval.comp %>% round(2)
...
plt.km.05.16.actual <-ggsurvplot_df(
  ...
  scale_color_manual(values = c("No DM" = "#1F78B4",
                                ...
                                "DM: OGTT only" = "#1B9E77")) +
  #add_legend_title("Diabetes\nDiagnosis") + 
  annotate("text",x=20,y=0.85,label=paste0("* HR=",hr.1,"\n","p = ",pval.1),color="#D81B60") + 
  annotate("text",x=20,y=0.70,label=paste0("+ HR=",hr.2,"\n","p = ",pval.2),color="#1B9E77") +
  annotate("text",x=20,y=0.55,label=paste0("∆ HR=",hr.3,"\n","p = ",pval.3),color="black") +
...
plt.km.05.16.actual <-ggsurvplot_df(
  ...
  #linetype = "strata", # This correctly tells it to use the 'strata' column for linetypes
  size=2
) + ylim(0.6,1) + xlim(0,250) + theme.1 + theme_bw() +   
  scale_color_manual(values = c("No DM" = "lightgrey",
                                "DM: A1c/FPG" = "darkgrey",
                                "DM: OGTT only" = "black"),
                     breaks= c("No DM","DM: A1c/FPG","DM: OGTT only")) +
  #add_legend_title("Diabetes\nDiagnosis") + 
  annotate("text",x=25,y=0.78,label=paste0("* HR=",hr.1,"\n","p = ",pval.1),color="black") + 
  annotate("text",x=25,y=0.70,label=paste0("+ HR=",hr.2,"\n","p = ",pval.2),color="black") +
  annotate("text",x=25,y=0.62,label=paste0("∆ HR=",hr.3,"\n","p = ",pval.3),color="black") +
  geom_segment(data=df.seg.1,aes(x=x1,xend=x2,y=y1,yend=y2),size=1) + coord_cartesian(xlim = c(0,210)) +
  annotate("text",x=df.seg.1$x1,y=1.01*max(df.seg.1$y1,df.seg.1$y2),label="*",size=5) +
  geom_segment(data=df.seg.2,aes(x=x1,xend=x2,y=y1,yend=y2),size=1) +  
  annotate("text",x=df.seg.2$x1,y=1.01*max(df.seg.2$y1,df.seg.2$y2),label="+",size=5) +
...
saveRDS(object = plt.km.05.16.actual,file="~/Documents/NHANES-DMDx/plt.km.05.16.actual.rds")
...
ggsave(plot=plt.km.05.16.actual + 
               ...
               theme.1 + coord_cartesian(xlim = c(0, 200)),filename="figures/KM-2005-2016.bmp",width=7,height=5,units=c("in"),create.dir = TRUE)
...
ggsave(plot = g2,filename = "figures/Surv-DMDx-cause.png",width=7,height=7,units="in")
ggsave(plot = g2,filename = "figures/1.Surv-DMDx-cause.bmp",width=7,height=7,units="in")
ggsave(plot = g2,filename = "figures/1.Surv-DMDx-cause.pdf",width=7,height=7,units="in",device=cairo_pdf)
