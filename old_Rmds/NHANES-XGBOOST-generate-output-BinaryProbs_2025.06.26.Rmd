...
f.clean.data.for.xgboost <- function(df.small){
  ...
  office_train <- df.small %>% select(all_of(var.included)) %>% filter(Year < 2013) #training(office_split)
  office_test <- df.small %>% select(all_of(var.included)) %>% filter(Year >= 2013) # testing(office_split)
  ...
  office_train = office_train %>% select(-c(sample.ogtt.weight,Year,two,DM)),
  office_test = office_test %>% select(-c(sample.ogtt.weight,Year,two,DM))
 ))
}
...
model_configs <- list(
  ...
  SOC = list(
    mutate_expr = ~ select(.,all_of(Var.SOC)),
   pred_col = "Pred.SOC"
  ),
  SOC.Fast = list(
    mutate_expr = ~ select(.,all_of(Var.SOC.fast)),
...
```{r}
office_test %>% filter(fpg<126) %>% ggplot()+ geom_point(aes(x=a1c,y=two,shape=DM,color=Pred.Full,size=a1c)) + scale_color_viridis_c(direction=-1)

office_test  %>% ggplot()+ geom_point(aes(x=a1c,y=Pred.Full,shape=DM,color=fpg,size=two)) + scale_color_viridis_c(direction=-1)
office_test  %>% ggplot()+ geom_point(aes(x=fpg,y=Pred.Full,shape=DM,color=a1c,size=two)) + scale_color_viridis_c(direction=-1)
office_test  %>% ggplot()+ geom_point(aes(x=two,y=Pred.Full,shape=DM,color=a1c,size=a1c)) + scale_color_viridis_c(direction=-1)
