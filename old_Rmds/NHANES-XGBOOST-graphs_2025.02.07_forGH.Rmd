...
df <- df.master %>% 
  ...
  dplyr::rename(Ever.Told.Overweight = MCQ080, Cuff.Size=BPACSZ) %>% 
...
df %>% nrow()
df$PHDSESN %>% table() 
df$PHDSESN %>% table() %>% sum()
### Morning: 26028
### Afternoon: 20328
### Evening: 9769
### NA: 4811
is.na(df$PHDSESN) %>% sum()

print('2007-2015')
df %>% filter(Year > 2005) %>% nrow()
df %>% filter(grepl('orning',df$PHDSESN)) %>% nrow()
df %>% filter(!grepl('orning',df$PHDSESN)) %>% nrow()
df %>% filter(Age>11 & grepl('orning',df$PHDSESN)) %>% nrow()
df %>% filter(Age>11 & grepl('orning',df$PHDSESN)) %>% nrow()
df %>% filter(Age>11 & grepl('orning',PHDSESN) & grepl('OGTT',ogtt.incomplete) ) %>% nrow()
df %>% filter(Age>11 & grepl('orning',PHDSESN) & grepl('OGTT',ogtt.incomplete) )%>% filter(!is.na(two)) %>% pull(two) %>% table() %>% sum()
df %>% filter(Age>11 & grepl('orning',PHDSESN) & grepl('OGTT',ogtt.incomplete) )%>% filter(!is.na(two),!is.na(a1c)) %>% pull(two) %>% table() %>% sum()
df %>% filter(Age>11 & grepl('orning',PHDSESN) & grepl('OGTT',ogtt.incomplete) )%>% filter(!is.na(two),!is.na(a1c),!is.na(sample.ogtt.weight)) %>% pull(two) %>% table() %>% sum()

print('2007-2015 Exclusion')
df %>% filter(Year < 2007) %>% nrow()
df %>% filter(Year > 2005,Age < 12) %>% nrow()
df %>% filter(Year > 2005,Age>11,!grepl('orning',df$PHDSESN)) %>% nrow()
df %>% filter(Year > 2005,Age>11 & grepl('orning',df$PHDSESN)) %>% nrow()
df %>% filter(Year > 2005,Age>11 & grepl('orning',df$PHDSESN)) %>% nrow()
df %>% filter(Year > 2005 & Age>11 & grepl('orning',PHDSESN) & !grepl('OGTT',ogtt.incomplete) ) %>% nrow()
df %>% filter(Year > 2005 & Age>11 & grepl('orning',PHDSESN) & grepl('OGTT',ogtt.incomplete) )%>% filter(!is.na(two)) %>% pull(two) %>% table() %>% sum()
df %>% filter(Year > 2005,Age>11 & grepl('orning',PHDSESN) & grepl('OGTT',ogtt.incomplete) )%>% filter(!is.na(two),is.na(a1c)) %>% pull(two) %>% table() %>% sum()
df %>% filter(Year > 2005,Age>11 & grepl('orning',PHDSESN) & grepl('OGTT',ogtt.incomplete) )%>% filter(!is.na(two),!is.na(a1c),!is.na(sample.ogtt.weight)) %>% pull(two) %>% table() %>% sum()

df %>% filter(Year > 2005 & Age>11 & grepl('orning',PHDSESN) & !grepl('OGTT',ogtt.incomplete) ) %>% filter(is.na(ogtt.incomplete)) %>% nrow()
df %>% filter(Year > 2005 & Age>11 & grepl('orning',PHDSESN) & !grepl('OGTT',ogtt.incomplete) ) %>% pull(ogtt.incomplete) %>% table() %>% sum()

df %>% mutate(ogtt.incomplete=ifelse(is.na(ogtt.incomplete),"NA",ogtt.incomplete)) %>%  select(ogtt.incomplete,Year) %>% table()
df$ogtt.incomplete %>% table()
is.na(df$ogtt.incomplete) %>% sum()

print('2005')
df %>% filter(Year==2005) %>% nrow()
df %>% filter(Year==2005,Age < 12) %>% nrow()
df %>% filter(Year==2005 & Age > 11 & !grepl('orning',PHDSESN)) %>% nrow()
df %>% filter(Year==2005 & Age > 11 & grepl('orning',PHDSESN)) %>% filter(is.na(two)) %>% nrow()
df %>% filter(Year==2005 & Age > 11 & grepl('orning',PHDSESN)) %>% filter(!is.na(two),is.na(a1c)) %>% nrow()
df %>% filter(Year==2005 & Age > 11 & grepl('orning',PHDSESN)) %>% filter(!is.na(two),!is.na(a1c),is.na(sample.ogtt.weight)) %>% nrow()

df %>% filter(Year==2005) %>% filter(is.na(two)) %>% pull(two) %>% length()
df %>% filter(Year==2005) %>% filter(!is.na(two),!is.na(a1c)) %>% pull(two) %>% table() %>% sum()
df %>% filter(Year==2005) %>% filter(!is.na(two),!is.na(a1c),!is.na(sample.ogtt.weight)) %>% pull(two) %>% table() %>% sum()
...
df.master$GTDCODE %>% length()
df.master$GTDCODE %>% table() %>% data.frame()  #%>% mutate(OGTT.Complete = `OGTT Complete` + `OGTT complete`)
...
```{r}
plot.import.full<- df.imp %>% arrange(Gain) %>% mutate(Feature=factor(Feature,levels=Feature)) %>% ggplot() + geom_col(aes(x=Feature,y=Gain))  + coord_flip() + labs(title="Importance Plot - Top 20 features") + theme(plot.title = element_text(hjust = 0.5,size=8),
...
ggsave('~/Dropbox/K23/Importance.png', plot.import.full, width = 3.2, height = 2.35, units = "in")
```


```{r}
df.imp %>% arrange(desc(Gain)) %>% mutate(Feature=factor(Feature,levels=Feature)) 
```
...
print('')
paste("Pre-Diabetes Dx by Both:", df.sampled %>% filter((a1c>=5.7 & a1c<6.5 )| (two<200 & two>=140)) %>% nrow() / 100000 ) 
paste("Pre-Diabetes Dx by OGTT:", df.sampled %>% filter((two<200 & two>=140)) %>% nrow() / 100000 ) 
paste("Pre-Diabetes Dx by A1c:", df.sampled %>% filter((a1c>=5.7 & a1c<6.5 )) %>% nrow() / 100000 ) 
paste("Pre-Diabetes Dx by A1c not OGTT:", df.sampled %>% filter((a1c>=5.7 & a1c<6.5 ) & two < 140) %>% nrow() / 100000 ) 
paste("Pre-Diabetes Dx by OGTT not A1c:", df.sampled %>% filter(a1c<5.7 & (two<200 & two>=140)) %>% nrow() / 100000 ) 
...
```{r}
x0.pdm.roc.fpg %>% arrange(cutoff)
```

```{r}
plt.roc.manu.fpg<- roc.dm.cohort %>% bind_rows(roc.pdm.cohort) %>% # %>% group_by(Cohort,Stat) %>% mutate(Best = tpr - fpr) %>% filter(Best==max(Best)) ) %>% 
#bind_rows(roc.pdm.cohort %>% bind_rows(roc.pdm.cohort %>% group_by(Cohort,Stat) %>% mutate(Best = tpr - fpr) %>% filter(Best==max(Best)) ) ) %>% 
 filter(Cohort=="All") %>% 
  mutate(Diagnosis=if_else(Dx=="DM","Diabetes","Pre-Diabetes")) %>% filter(Stat=="Min" |Stat=="A1c" | Stat=="FPG") %>% mutate(Stat=if_else(Stat=="Min","Model",Stat)) %>% 
 ggplot(aes(x=fpr,y=tpr,linetype=Stat,shape=Stat,color=Diagnosis)) + geom_line() +
...
#                  aes(label=paste0("(",round(fpr,2),', ',round(tpr,2),")"),x=fpr,y=tpr),size=3) + 
  geom_abline(aes(slope=1,intercept=0),linetype="dotted") + labs(title="Receiver Operator Curves",linetype="Method",shape="Method") + xlab("1-Specificity") + ylab("Sensitivity") + 
  scale_color_manual(values=c("Diabetes"="maroon","Pre-Diabetes"="darkgreen")); print(plt.roc.manu.fpg)

ggsave('~/Dropbox/NHANES manuscript/ROC-PreDM-DM-MinvA1cvFPG.jpg',plot=plt.roc.manu.fpg)
```
...
```{r For K23}
plot.roc.K23<- roc.dm.cohort %>% bind_rows(roc.dm.cohort %>% group_by(Cohort,Stat) %>% mutate(Best = tpr - fpr) %>% filter(Best==max(Best)) ) %>% 
bind_rows(roc.pdm.cohort %>% bind_rows(roc.pdm.cohort %>% group_by(Cohort,Stat) %>% mutate(Best = tpr - fpr) %>% filter(Best==max(Best)) ) ) %>% 
 filter(Cohort=="All") %>% 
  mutate(Diagnosis=if_else(Dx=="DM","DM","Pre-DM")) %>% filter(Stat=="Min" |Stat=="A1c") %>% mutate(Stat=if_else(Stat=="Min","Model",Stat)) %>% 
 ggplot(aes(x=fpr,y=tpr,linetype=Stat,shape=Stat,color=Diagnosis)) + geom_line() +
 #geom_point(data=. %>% filter(!is.na(Best)),aes(label=paste0(round(fpr,2),', ',round(tpr,2))),size=2) +   
  #geom_text_repel(data=. %>% filter(!is.na(Best)),aes(label=paste0("(",round(fpr,2),', ',round(tpr,2),")"),x=fpr,y=tpr),size=3) + 
  geom_abline(aes(slope=1,intercept=0),linetype="dotted") + 
  labs(linetype="Method",shape="Method") + xlab("1-Specificity") + ylab("Sensitivity") + 
  scale_color_manual(values=c("DM"="maroon","Pre-DM"="darkgreen")) + 
        theme(axis.text=element_text(size=8),
              axis.title=element_text(size=8),
              legend.text=element_text(size=8,margin=margin(l=unit(0.001,'in'))),
              legend.title=element_text(size=8,margin=margin(b=unit(0.001,'in'))),
              legend.margin = unit(0.05,'in'),
              plot.title = element_text(size=8),
              legend.spacing.x = unit(0.001, 'in'),              
              legend.spacing.y = unit(0.001, 'in'),
              legend.key.size = unit(0.15, "in"),
              legend.box.spacing=unit(0.01,"in"),
              legend.key.spacing.x=unit(0.005,"in"),
              legend.key.spacing.y=unit(0.005,"in")) ; print(plot.roc.K23)

ggsave('~/Dropbox/K23/ROC.jpg',plot=plot.roc.K23,units=c("in"),height=1.3,width=2.2)
```
...
yy <- office_test.All
x0.yy.pdm <- yy.pdm <- factor(yy$two >=140 & yy$two<200)
x0.pdm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.pdm))
...
yy <- office_test.WC  
x3.yy.pdm <- yy.pdm <- factor(yy$two >=140 & yy$two<200)
x3.pdm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.pdm))
...
x4.pdm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.pdm))
...
x5.pdm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.pdm))
...
x6.pdm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.pdm))
...
yy <- office_test.BMI
yy.dm <- factor(yy$two >= 200)
x2.dm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.dm))
...
yy <- office_test.WC  
yy.dm <- factor(yy$two >= 200)
x3.dm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.dm))
...
yy <- office_test.FIB4
yy.dm <- factor(yy$two >= 200)
x4.dm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.dm))
...
yy <- office_test.GGT
yy.dm <- factor(yy$two >= 200)
x5.dm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.dm))
...
yy <- office_test.SBP
yy.dm <- factor(yy$two >= 200)
x6.dm.auroc.fpg <- auc(AUC::roc(yy$fpg, yy.dm))
...
combined_auroc_table <- data.frame(
  ...
  AUROC_FPG_PDM = round(c(x0.pdm.auroc.fpg, x1.pdm.auroc.fpg, x2.pdm.auroc.fpg, x3.pdm.auroc.fpg, 
                           x4.pdm.auroc.fpg, x5.pdm.auroc.fpg, x6.pdm.auroc.fpg), 3),
  AUROC_FPG_DM  = round(c(x0.dm.auroc.fpg, x1.dm.auroc.fpg, x2.dm.auroc.fpg, x3.dm.auroc.fpg, 
                           x4.dm.auroc.fpg, x5.dm.auroc.fpg, x6.dm.auroc.fpg), 3),
...
```{r}
office_test.All %>% mutate(a1c_range=cut(a1c,c(4,8,12,16))) %>% ggplot()+ geom_point(aes(x=a1c,y=two,color=a1c),alpha=0.5) + geom_abline(aes(slope=1,intercept=0)) + ylim(c(50,500)) + scale_color_viridis()


plt.calibration<-office_test.All %>% mutate(a1c_range=cut(a1c,c(4,8,12,16)))    %>% 
  ggplot()+ 
  geom_point(aes(x=Pred.MIN,y=two),alpha=1) + 
  geom_point(data=office_test.All %>% filter(a1c<6.5),aes(x=Pred.MIN,y=two,color=a1c),alpha=1) +  
  geom_abline(aes(slope=1,intercept=0))+ xlim(c(50,500)) + ylim(c(50,500)) + scale_color_viridis(name='Hemoglobin\nA1c (%)') + 
  xlab("Predicted 2-hour Glucose") + ylab("Measured 2-hour Glucose (mg/dL)");plot(plt.calibration)

ggsave(file='~/Dropbox/NHANES manuscript/Calibration.jpg',plot=plt.calibration)
