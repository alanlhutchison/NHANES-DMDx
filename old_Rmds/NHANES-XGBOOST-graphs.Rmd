...
df.desc.stats <-df.small %>% select(Var.MIN,Year) %>% mutate(Year.2015 = ifelse(Year > 2013,"2015-2016","2005-2014")) %>% 
...
```{r}
df.imp <- xgb.importance(feature_names = colnames(xgb.model.full),model=xgb.model.full) %>% select(Feature,Gain)  %>% mutate(Gain=round(Gain,3)) %>% head(20) %>% mutate(Sum=sum(Gain)) %>%
  mutate(Feature=str_replace(Feature,'fpg','Fasting Plasma\nGlucose'),
         Feature=str_replace(Feature,'a1c','Hemoglobin A1c'),
         Feature=str_replace(Feature,'tgy','Triglycerides'),
         Feature=str_replace(Feature,'SBP','Systolic Blood\nPressure'),
         Feature=str_replace(Feature,'u.alb','Urine Albumin'),         
         Feature=str_replace(Feature,'Height.Stand','Height'),         
         Feature=str_replace(Feature,'insulin','Insulin'),
         Feature=str_replace(Feature,'Seg.Neut.perc','% Segmented\nNeutrophils'),
         Feature=str_replace(Feature,'u.cr','Urine Creatinine'),
         Feature=str_replace(Feature,'Lymph.Perc','% Lymphocytes'),
         Feature=str_replace(Feature,'Poverty.Ratio','Poverty Ratio'),
         Feature=str_replace(Feature,'pulse','Pulse'),
         Feature=str_replace(Feature,'wbc','White Blood\nCell Count'),
         Feature=str_replace(Feature,'choles','Cholesterol'),
         Feature=str_replace(Feature,'^Waist.Circ','Waist\nCircumference'),
         Feature=str_replace(Feature,'^ast','AST'),
         Feature=str_replace(Feature,'^DBP','Diastolic Blood\nPressure'),
         Feature=str_replace(Feature,'^ggt','GGT'),
         Feature=str_replace(Feature,'^cr','Creatinine'),
  ) %>%  
...
bind_rows(roc.pdm.cohort %>% bind_rows(roc.pdm.cohort %>% group_by(Cohort,Stat) %>% mutate(Best = tpr - fpr) %>% filter(Best==max(Best)) ) ) %>% 
 ...
 geom_point(data=. %>% filter(!is.na(Best)),
  ...
  geom_text_repel(data=. %>% filter(!is.na(Best)),
                  aes(label=paste0("(",round(fpr,2),', ',round(tpr,2),")"),x=fpr,y=tpr)) + 
  geom_abline(aes(slope=1,intercept=0),linetype="dotted") + labs(title="Receiver Operator Curves",linetype="Method",shape="Method") + xlab("Specificity") + ylab("Sensitivity") + 
