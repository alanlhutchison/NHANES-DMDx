...
office_train <- df_full %>% filter(Year > 2005)
office_test <- df_full %>% filter(Year == 2005)
...
```{r}
paste("Of",
      nrow(office_pre) + nrow(office_train) + nrow(office_test), "NHANES subjects from 1999 to 2016, we identified", 
      nrow(office_train %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.ogtt.weight))) + 
  nrow(office_test %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.fasting.weight))),
  "subjects with FPG, A1c, or OGTT results (",
  df_full %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.ogtt.weight)) %>% left_join(mortality_clean,join_by(SEQN==seqn)) %>% filter(eligstat==1) %>% nrow()
,
  "with mortality data) and",
  nrow(df_full) - 
  nrow(office_train %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.ogtt.weight))) -
  nrow(office_test %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.fasting.weight))),
  "other subjects (",
  df_full %>% left_join(mortality_clean,join_by(SEQN==seqn)) %>% filter(eligstat==1) %>% nrow() -
  df_full %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.ogtt.weight)) %>% left_join(mortality_clean,join_by(SEQN==seqn)) %>% filter(eligstat==1) %>% nrow()
,
  "with mortality data)." )
nrow(office_pre)
...
nrow(office_pre) + nrow(office_train) + nrow(office_test) - 
  nrow(office_train %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.ogtt.weight))) - 
  nrow(office_test %>% filter(!is.na(two),!is.na(fpg),!is.na(a1c),!is.na(sample.fasting.weight),!is.na(sample.fasting.weight)))
...
mortality_clean %>% filter(Year >2003,Year < 2017) %>% nrow()
```
...
file.list.models <- "files/NHANES-All-xgb.models_all_2026.01.08_2005test.Rda"
...
plt.km.BWF <-ggsurvplot_df(
  ...
  #linetype = "strata", # This correctly tells it to use the 'strata' column for linetypes
  size=2
) + ylim(0.38,1) + theme.1 + theme_grey() +   
  scale_color_manual(values = c("No DM: Predicted No DM" = "#1F78B4",
                                "No DM: Predicted DM" = "#1B9E77",
                                "DM: Predicted DM" = "#D81B60")) +
  #add_legend_title("Diabetes\nDiagnosis") + 
  annotate("text",x=75,y=0.48,label=paste0("A1c<6.5% and FPG<126\nPredicted DM"),color="#1B9E77",size=2,lineheight=0.8) + 
  annotate("text",x=75,y=0.39,label=paste0("A1c≥6.5% or FPG≥126\nPredicted DM"),color="#D81B60",size=2,lineheight=0.8) +
  annotate("text",x=75,y=0.57,label=paste0("A1c<6.5% and FPG<126\nPredicted No DM"), color="#1F78B4",size=2,lineheight=0.8) 
  # geom_segment(data=df.seg.1,aes(x=x1,xend=x2,y=y1,yend=y2),size=1) + coord_cartesian(xlim = c(0,260)) +
  # annotate("text",x=df.seg.1$x1,y=1.01*max(df.seg.1$y1,df.seg.1$y2),label="*",size=5) +
  # geom_segment(data=df.seg.2,aes(x=x1,xend=x2,y=y1,yend=y2),size=1) +  
  # annotate("text",x=df.seg.2$x1,y=1.02*max(df.seg.2$y1,df.seg.2$y2),label="+",size=5) +
...
plt.km.BWF + labs(tag="B")

plt.shap.summ.soc.fast.10 + labs(tag="A") + theme(legend.position=c(.9,.5))

ggsave(plot=
         grid.arrange(ncol=2,nrow=1,
                      plt.shap.summ.soc.fast.10 + labs(tag="A") + 
                      ...
                      plt.km.BWF + labs(tag="B") +
                        theme(legend.position ="none",
                              ...
                              axis.title=element_text(size=8),
                              plot.title = element_text(hjust = 0.5,size=8))),
                      ,filename="figures/Shap.10-Mort.bmp",width=4,height=1.8,units = c("in"))
...
```{r FROM DMDx-GraphModelREsults_2025.09/}
plt.roc.results.few.fasting.dm<- roc_results %>% filter(Dx=="DM") %>% filter(Type=="FPG.Fixed" | Type=="FPG" | Type=="A1c/FPG+") %>% 
  ...
  scale_color_manual(values=c("A1c"="darkgreen","FPG"="green","FPG+A1c"="brown","NonFasting"="magenta","Full"="orange","Min"="red","A1c+"="darkblue","A1c/FPG+"="blue","Test None (B)"="black","FPG.Fixed"="green","A1c.Fixed"="darkgreen")) +
    scale_linetype_manual(values=c("A1c"="longdash","FPG"="solid","FPG+A1c"="solid","NonFasting"="longdash","Full"="solid","Min"="solid","A1c+"="longdash","A1c/FPG+"="solid","Test None (B)"="dotted","A1c.Fixed"="dotted","FPG.Fixed" ="dotted")) 
 #guides(
...
grid.arrange(ncol=3,nrow=1,
                      ...
                      plt.shap.summ.soc.fast.10 + ggtitle("A1c/FPG+ Model") + labs(tag="A") + 
                        theme(legend.position =c(.7,.35),
                              plot.tag=element_text(size=8),
                              axis.text=element_text(size=8),
                              axis.title=element_text(size=8),
                              ...
                              legend.key.size = unit(.05, "in"), 
                              plot.title = element_text(hjust = 0.5,size=8)),
                    plt.roc.results.few.fasting.dm + labs(tag="B")+ 
                        theme(legend.position =c(.75,.22),
                              ...
                              legend.key.size = unit(.05, "in"), 
                              plot.title = element_text(hjust = 0.5,size=8)),
                      plt.km.BWF + labs(tag="C") +
                        theme(legend.position ="none",
                              plot.tag=element_text(size=8),                              
                              axis.text=element_text(size=8),
                              axis.title=element_text(size=8),
                              plot.title = element_text(hjust = 0.5,size=8)))
#ggsave(plot=                      ,filename="~/Dropbox/Dec2025-CDA-submissions/AASLD-CTORA/ROC-Shap.10-Mort.bmp",width=6,height=2,units = c("in"))
