...
ggsave(plot=plt.shap.lines.full,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-lines-Full.bmp",width=7,height=7,units = c("in"))
...
# 
# ggsave(filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-Summary.bmp",plot=plt.shap.summ)
...
#ggsave(plot=plt.shap.lines,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-lines.bmp",width=7,height=7,units = c("in"))
...
plt.shap.lines.full <- shap.dat.full.2 %>% ggplot(aes(x=Value,y=Shap)) + geom_point(size=0.5) + geom_smooth() + facet_wrap(~Feature,scales = "free") + ggtitle("Full Model")  + theme.1 +  ylab("SHAP Values");plot(plt.shap.lines.soc)
...
ggsave(plot=plt.shap.summ.full,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-Summ-Full.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.lines.soc,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-lines-Soc.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.summ.soc,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-Summ-Soc.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.lines.soc.fast,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-lines-Soc.Fast.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.summ.soc.fast,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap-Summ-Soc.Fast.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=g,filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Shap.Full.AB.bmp",width = 7,height = 7, units = "in")
...
#ggsave('~/Dropbox/NHANES manuscript/figures/Pre-DM/Feature-Importance.bmp', plot.import.full, width = 4, height = 3, units = "in")
...
ggsave(plot=plt.cohort.boxplots,file='~/Dropbox/NHANES manuscript/figures/Pre-DM/Cohort-Boxplots.bmp')
```
...
plt.cohorts %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/Cohort-6.bmp",height=10,width=7,units=c("in")); plot(plt.cohorts)
...
plt.egfr.only %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/A1c-2hG-eGFR-only.bmp",height=10,width=7,units=c("in"))
...
plot_reliability_diagram(office_test, "Pred.SOC.Fast", "DM",
                           paste0("Isotonic Regression (", "SOC.Fast", ")"), "darkblue") %>% ggsave(file='~/Dropbox/NHANES manuscript/figures/Pre-DM/Calibration_plot_Iso_SOC-Fast.bmp')
}
```
...
office_test %>% ggplot()+ geom_histogram(aes(x=Pred.SOC,fill=as.factor(PDM)))
...
#office_test %>% mutate(PDM=PDM==TRUE) %>% cal_plot_windowed(PDM,Pred.Full) %>% ggsave(filename="~/Dropbox/NHANES manuscript/figures/Pre-DM/Binary-Calibration.bmp")
office_test %>% mutate(PDM=PDM==TRUE) %>% cal_plot_breaks(truth=PDM,estimate=Pred.SOC,num_breaks=5) + theme_grey()
office_test %>% mutate(PDM=PDM==TRUE) %>% cal_plot_windowed(truth=PDM,estimate=Pred.SOC,window_size=0.2) + theme_grey()

calculate_brier_score <- function(predicted_probs, true_labels) {
  mean((predicted_probs - true_labels)^2, na.rm = TRUE)
}

calculate_brier_score(office_test$Pred.SOC,office_test$PDM==TRUE)
calculate_brier_score(office_test$Pred.SOC.Fast,office_test$PDM==TRUE)


office_test %>% ggplot() + geom_point(aes(x=Pred.SOC,y=Pred.SOC.Fast,color=PDM),alpha=0.7)

val.prob.out <- rms::val.prob(office_test$Pred.SOC.Fast,office_test$PDM==TRUE,g=10,logistic.cal=FALSE,smooth=FALSE) 
rms::val.prob(office_test$Pred.SOC,office_test$PDM==TRUE,g=20)
rms::val.prob(office_test$Pred.FIB4,office_test$PDM==TRUE,g=20)


office_test %>% mutate(PDM=PDM==TRUE) %>%  arrange(Pred.SOC.Fast) %>% mutate(sum=cumsum(PDM)/sum(PDM)) %>% select(Pred.SOC.Fast,sum) %>% ggplot() + geom_line(aes(x=Pred.SOC.Fast,sum))
...
office_test %>% mutate(PDM=PDM==TRUE) %>% cal_plot_breaks(truth=PDM,estimate=Pred.SOC,num_breaks=5) + theme_grey()
office_test %>% mutate(PDM=PDM==TRUE) %>% cal_plot_windowed(truth=PDM,estimate=Pred.SOC,window_size=0.2) + theme_grey()
office_test %>% mutate(PDM=PDM==TRUE) %>% cal_plot_breaks(truth=PDM,estimate=Pred.SOC.Fast,num_breaks=5) + theme_grey()
office_test %>% mutate(PDM=PDM==TRUE) %>% cal_plot_windowed(truth=PDM,estimate=Pred.SOC.Fast,window_size=0.2) + theme_grey()
...
plt.calibration.quantile <- office_test %>% mutate(bin=cut_number(Pred.SOC.Fast, n = 20)) %>% group_by(bin) %>% reframe(med=median(Pred.SOC.Fast),freq=sum(PDM==TRUE)/sum(!is.na(PDM))) %>% ggplot() + geom_point(aes(x=med,y=freq)) + geom_line(data=data.frame(x=c(0,1),y=c(0,1)),aes(x=x,y=y),linetype="dashed") +  geom_rug(data=office_test,aes(x=Pred.SOC.Fast),alpha=0.6) + xlab("Median of Probaility Quantile") + ylab("Frequency of Diabetes Diagnosis") + xlim(0,1) + ylim(0,1) + ggtitle("Frequency of Diagnosis by Quantile for SOC-Fasting Model") + theme.1; plot(plt.calibration.quantile)

ggsave(plt.calibration.quantile,file="~/Dropbox/NHANES manuscript/figures/Pre-DM/Quantile-calibration-Pred.SOC.Fasting.bmp")
...
ot2 <- ot %>% mutate(quantile=cut_number(a1c,5))
...
ggsave(file='~/Dropbox/NHANES manuscript/figures/Pre-DM/Calibration_cohorts.bmp',plot=plt.calibration.cohorts)
...
df.stats$Type <- c(rep("PDM",3),rep("DM",3))
...
colnames(roc_results) <- c("PDM|Cutoff","PDM|FPR","PDM|TPR","DM|Cutoff","DM|FPR","DM|TPR")
...
pROC::roc(predictor=office_test$fpg,response=(x0.igt==TRUE & office_test$fpg>=100 )) #%>% pROC::coords(.,100,input="threshold")
...
pROC::roc(predictor=office_test$fpg,response=(x0.igt==TRUE |( office_test$fpg>=100) ))  %>% pROC::coords(.,x="best",ret=c("threshold","specificity","sens"))
...
df.stats$Type <- c(rep("PDM",4),rep("DM",4))
...
c("Type"="Pre-PDM","Vars"="FPG.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$fpg,response=(x0.igt)) %>% 
            pROC::coords(.,x=100,input="threshold",ret=c("threshold","specificity","sensitivity")) ),
c("Type"="PDM","Vars"="FPG.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$fpg,response=(x0.pphg)) %>% 
            pROC::coords(.,x=126,input="threshold",ret=c("threshold","specificity","sensitivity")) ),

c("Type"="Pre-PDM","Vars"="A1c.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$a1c,response=(x0.igt)) %>% 
            pROC::coords(.,x=5.7,input="threshold",ret=c("threshold","specificity","sensitivity"))),
c("Type"="PDM","Vars"="A1c.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$a1c,response=(x0.pphg)) %>% 
...
roc_results %>% ggplot() + 
  ...
  guides(
    ...
    color = "none") + 
  scale_linetype_discrete(labels=c("DM","PDM"))  ; print(plt.roc.results) 
...
plt.roc.results.few <- roc_results %>% filter(Type=="FPG" | Type=="A1c" | Type =="NonFasting" | Type=="Min")  %>% ggplot() +
  ...
  guides(
    ...
    shape = guide_legend(override.aes = list(linetype = c(1,2))), # Remove the separate shape legend
    color = "none")+
  scale_linetype_discrete(labels=c("DM","PDM")); print(plt.roc.results.few)
```
...
roc_results %>% filter(Dx=="PDM") %>% 
  rbind(data.frame(Type="Test All (B)",Type2="TA.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>% 
  rbind(data.frame(Type="Test None (B)",Type2="TN.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>%   
  rbind(data.frame(Type="A1c.Fixed",Type2="AF.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>% 
  rbind(data.frame(Type="FPG.Fixed",Type2="FF.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>%   
  
  ggplot() + 
  geom_line(aes(x=FPR,y=TPR,color=Type,linetype=Type)) +
  geom_point(data=roc_results.2 %>% filter(Dx=="PDM"),aes(x=FPR,TPR,color=Type2))+
...
roc_results %>% filter(Dx=="PDM") %>% filter(Type!="FPG-A1c" & Type!="NonFasting" & Type!="Min") %>% 
  rbind(data.frame(Type="Test All (B)",Type2="TA.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>% 
  #rbind(data.frame(Type="Test None (B)",Type2="TN.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>%   
  rbind(data.frame(Type="A1c.Fixed",Type2="AF.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>% 
  rbind(data.frame(Type="FPG.Fixed",Type2="FF.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>%     
...
roc_results %>% filter(Dx=="PDM") %>% filter(Type=="FPG.Fixed" | Type=="FPG" | Type=="A1c/FPG+") %>% 
  rbind(data.frame(Type="Test All (D)",Type2="TA.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>% 
  rbind(data.frame(Type="FPG.Fixed",Type2="FF.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>%     
  ggplot() + 
  geom_line(aes(x=FPR,y=TPR,color=Type,linetype=Type)) +
  geom_point(data=roc_results.2 %>% filter(Dx=="PDM" & Type2=="FPG.Fixed"),aes(x=FPR,TPR,color=Type2))+
  labs(color="Model",linetype="Model",shape="Model") + 
  xlab('False Positive Rate') + 
  ylab('True Positive Rate')  + 
  theme(plot.title = element_text(hjust = 0.5))  + 
...
roc_results %>% filter(Dx=="PDM") %>% filter(Type=="A1c.Fixed" | Type=="A1c" | Type=="A1c+") %>% 
  rbind(data.frame(Type="Test All (B)",Type2="TA.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>% 
  rbind(data.frame(Type="A1c.Fixed",Type2="AF.1",Dx="PDM",Cutoff=1,FPR=0,TPR=0)) %>% 
  ggplot() + 
  geom_line(aes(x=FPR,y=TPR,color=Type,linetype=Type)) +
  geom_point(data=roc_results.2 %>% filter(Dx=="PDM" & Type2=="A1c.Fixed"),aes(x=FPR,TPR,color=Type2))+
  labs(color="Model",linetype="Model",shape="Model") + 
  xlab('False Positive Rate') + 
  ylab('True Positive Rate')  + 
  theme(plot.title = element_text(hjust = 0.5))  + 
...
df.stats.table.t <-df.stats.table %>% filter(Type=="PDM") %>% select(-c(Type)) %>% pivot_wider(names_from = Stat,values_from = Value) %>% select(-c(Vars,Threshold)) %>% relocate(AUROC,Sensitivity,Specificity) %>% t() %>% as.data.frame() 

names(df.stats.table.t)  <- df.stats.table %>% filter(Type=="PDM") %>% pivot_wider(names_from = Stat,values_from = Value) %>% pull(Vars)
...
df.stats <- df.stats %>% mutate(Value=ifelse(Stat=="Specificity" & Type=="PDM" & Vars=="FPG.Fixed",1,Value))  
...
dput(data_df)
# structure(list(A1c.Fixed = c(NA, 0.37, 1), A1c = c(0.81, 0.58, 
# 0.91), `A1c+` = c(0.87, 0.72, 0.86), FPG.Fixed = c(NA, 0.48, 
# 1), FPG = c(0.93, 0.83, 1), `FPG+A1c` = c(0.98, 0.95, 1), `A1c/FPG+` = c(0.99, 
# 0.95, 1), Full = c(0.99, 0.95, 1)), class = "data.frame", row.names = c("AUROC", 
# "Sensitivity", "Specificity"))


#### Here we editing this table to match the Pre-DM results from the full model, just editing the Full=c(0.97,0.9,1)
data_df <- structure(list(A1c.Fixed = c(NA, 0.37, 1), A1c = c(0.81, 0.58, 
0.91), `A1c+` = c(0.87, 0.72, 0.86), FPG.Fixed = c(NA, 0.48, 
1), FPG = c(0.93, 0.83, 1), `FPG+A1c` = c(0.98, 0.95, 1), `A1c/FPG+` = c(0.99, 
0.95, 1), Full = c(0.97, 0.9, 1)), class = "data.frame", row.names = c("AUROC", 
"Sensitivity", "Specificity"))
...
df.dca.eval <- office_test %>% 
  mutate(A1c.Fixed = ifelse(a1c>=5.7,1,0)) %>% 
  mutate(FPG.Fixed = ifelse(fpg>=100,1,0)) %>% 
...
df.dca.eval <- df.dca.eval %>% mutate(pm.pdm.1=ifelse(pm.pdm.1>0.5,1,0)) %>% mutate(pm.pdm.2=ifelse(pm.pdm.2>0.5,1,0))

plot.dca.eval.dm <- dca_plot(df.dca.eval,"PDM", "pm.pdm", "")

plt.dca.dm<-plot(plot.dca.eval.dm + 
       coord_cartesian(xlim=c(-0.001,0.41),ylim = c(-.001,.6)) + 
       xlab("Risk needed to warrant extra testing")) +
  ylab("Net Benefit") +
  scale_x_continuous(breaks=c(0.0,0.1,0.2,0.3,0.4),labels=c("0%","10%","20%","30%","40%")) %>% 
  suppressWarnings()
...
df.dca.eval.fasting <- cbind(df.dca.eval,  fit_and_predict("PDM"))

df.dca.eval.few.fasting <- df.dca.eval.fasting %>% mutate(pm.pdm.1=ifelse(pm.pdm.1>0.5,1,0)) 


plot.dca.eval.dm.few.fast <- dca_plot(df.dca.eval.few.fasting ,"PDM", "pm.pdm", "")
...
df.dca.eval.nonfasting <- cbind(df.dca.eval,  fit_and_predict("PDM"))

df.dca.eval.few.nonfasting <- df.dca.eval.nonfasting %>% mutate(pm.pdm.1=ifelse(pm.pdm.1>0.5,1,0)) 


plot.dca.eval.dm.few.nonfast <- dca_plot(df.dca.eval.few.nonfasting ,"PDM", "pm.pdm", "")
...
plt.2 <- plt.roc.results.few.fasting.dm + annotation_custom(tableGrob(data_df[,4:6], theme = mytheme), xmin=0.32, xmax=0.9, ymin=-0.15, ymax=0.5)
...
grid.arrange(nrow=4,heights=c(4,1,4,1),
            ...
            arrangeGrob(ncol=2,
             ...
             legend.2) %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/ROC-DCA-figure-4.bmp")
...
grid.arrange(nrow=3,heights=c(4,1,1.2),
             ...
             ),top="Diagnosis of Pre-Diabetes") %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/ROC-DCA-figure-AB.bmp")
...
#              ),top="Diagnosis of Pre-Diabetes") %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/ROC-DCA-figure-AB-Few.bmp")
...
# Create outcome variables
df.dca.eval <- office_test %>% 
  mutate(PreDM = as.integer(two >= 140 ),
...
# Create outcome variables
df.dca.eval <- office_test %>% 
  mutate(PreDM = as.integer( (two >= 140 ) | ( a1c >=5.7) | (fpg>=100) ),
...
grid.arrange(nrow=3,heights=c(3,3,1),
            ...
            legend.2 ) %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/ROC-DCA-figure-soc.bmp")
...
grid.arrange(nrow=3,heights=c(3,3,1),
            ...
            legend.2 ) %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/ROC-DCA-figure-Few.bmp")
...
office_test$PDM <- as.factor(dplyr::between(office_test$two, 140, 199))
...
grid.arrange(nrow=4,heights=c(3,0.5,3,0.5),
  ...
  legend.2
) %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/Pre-DM/PPV-plots.bmp",height = 7,width = 7,units = "in")
