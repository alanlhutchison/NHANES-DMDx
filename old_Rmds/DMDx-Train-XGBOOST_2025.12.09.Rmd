...
office_train <- df_full %>% filter(Year > 2005, Year < 2015)
...
f.clean.data.for.xgboost <- function(df.small){
  ...
  office_train <- df.small %>% select(c(SEQN,all_of(var.included))) %>% filter(Year > 2005) #training(office_split)
  office_test <- df.small %>% select(c(SEQN,all_of(var.included))) %>% filter(Year == 2005) # testing(office_split)
...
office_test %>% select(SEQN,Pred.SOC.Fast,a1c,fpg,two,Year) 
``` 
