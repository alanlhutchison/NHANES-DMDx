...
ggplot()+ 
  geom_point(data=office_test %>% mutate(quantile=cut_number(a1c,9)),aes(x=Pred.Min,y=two,color=factor(quantile)),alpha=0.7) + 
  #geom_point(data=office_test %>% filter(a1c < 4.5 ),aes(x=Pred.Min,y=two,color=a1c),color='yellow',alpha=0.7) +  
  geom_line(data=data.frame(x=c(40,550),y=c(40,550)),aes(x=x,y=y),linetype="dashed")+ xlim(c(30,550)) + ylim(c(30,550)) + scale_color_viridis_d(name='Hemoglobin\nA1c (%)',direction=-1) + 
  xlab("Predicted 2-hour Glucose") + ylab("Measured 2-hour Glucose (mg/dL)")
...
cor(office_test$two,office_test$Pred.Min)^2
```
...
test.char.fixed <- as.data.frame(
c("Type"="Pre-DM","Vars"="FPG.Fixed",pROC::roc(predictor=office_test$fpg,response=(x0.igt)) %>% 
            pROC::coords(.,x=100,input="threshold",ret=c("threshold","specificity","sensitivity")) ),
c("Type"="DM","Vars"="FPG.Fixed",pROC::roc(predictor=office_test$fpg,response=(x0.pphg)) %>% 
            pROC::coords(.,x=126,input="threshold",ret=c("threshold","specificity","sensitivity")) ),

c("Type"="Pre-DM","Vars"="A1c.Fixed",pROC::roc(predictor=office_test$a1c,response=(x0.igt)) %>% 
            pROC::coords(.,x=5.7,input="threshold",ret=c("threshold","specificity","sensitivity"))),
c("Type"="DM","Vars"="A1c.Fixed",pROC::roc(predictor=office_test$a1c,response=(x0.pphg)) %>% 
            pROC::coords(.,x=6.5,input="threshold",ret=c("threshold","specificity","sensitivity")))
)
...
df.stats <- df.stats %>% cbind(
  data.frame(FPG.Fixed = c(0,0,0,100,1-stringent.threshold.pdm.fpg$FPR,round(stringent.threshold.pdm.fpg$TPR,2),
                         0,0,0,126,1-stringent.threshold.dm.fpg$FPR,round(stringent.threshold.dm.fpg$TPR,2)),
           A1c.Fixed = c(0,0,0,5.7,1-stringent.threshold.pdm$FPR,round(stringent.threshold.pdm$TPR,2),
                         0,0,0,6.5,1-stringent.threshold.dm$FPR,round(stringent.threshold.dm$TPR,2)))
  )
...
df.stats.table <- df.stats %>% filter(Stat=="Threshold" | Stat=="Specificity" | Stat=="Sensitivity" | Stat=="AUROC") %>% relocate(Type,Stat,A1c.Fixed,FPG.Fixed,A1c,FPG,`FPG-A1c`,FIB4,NonFasting,Min,Full)

df.stats.grob.Full <- df.stats.table %>% tableGrob(theme=mytheme)

df.stats.table.Few <- df.stats %>% filter(Stat=="Threshold" | Stat=="Specificity" | Stat=="Sensitivity" | Stat=="AUROC") %>% relocate(Type,Stat,A1c.Fixed,FPG.Fixed,A1c,A1c,FPG,`FPG-A1c`,FIB4,NonFasting,Min,Full) %>% select(Type,Stat,A1c.Fixed,FPG.Fixed,A1c,A1c,FPG,NonFasting,Min) 
