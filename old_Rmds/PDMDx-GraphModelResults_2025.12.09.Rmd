...
dir.string <-"files/"
date.string <- "2025.12.09-Binary_PDM"
filter.string <- "all"
...
df.small.resampled %>% filter(a1c>=5.7 & fpg >= 100 & two>=140) %>% nrow() / nrow(df.small.resampled)
df.small.resampled %>% filter(a1c>=5.7 & fpg < 100 & two>=140) %>% nrow() / nrow(df.small.resampled)
df.small.resampled %>% filter(a1c>=5.7 & fpg >= 100 & two<140) %>% nrow() / nrow(df.small.resampled)
df.small.resampled %>% filter(a1c>=5.7 & fpg < 100 & two<140) %>% nrow() / nrow(df.small.resampled)
...
paste("Diabetes Dx by Both:", df.small %>% filter(a1c>=5.7 & fpg >= 100 & two>=140) %>% nrow()  ) 
paste("Diabetes Dx by OGTT:", df.small %>% filter(a1c>=5.7 & two>=140 & fpg < 100) %>% nrow() / 1 ) 
paste("Diabetes Dx by A1c:", df.small %>% filter(a1c>=5.7) %>% nrow() / 1 ) 
paste("Diabetes Dx by A1c not OGTT:", df.small %>% filter(a1c>=5.7 & two < 140) %>% nrow() / 1 ) 
paste("Diabetes Dx by OGTT not A1c:", df.small %>% filter(a1c<5.7 & two >= 140) %>% nrow() / 1 ) 
...
ggsave(plot=plt.shap.lines.full,filename="figures-PDM/Shap-lines-Full-PDM.bmp",width=7,height=7,units = c("in"))
...
# 
# ggsave(filename="Shap-Summary-PDM.bmp",plot=plt.shap.summ)
...
#ggsave(plot=plt.shap.lines,filename="~/Dropbox/NHANES manuscript/figures/Shap-lines-PDM.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.summ.full,filename="figures-PDM/Shap-Summ-Full-PDM.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.lines.soc,filename="figures-PDM/Shap-lines-Soc-PDM.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.summ.soc,filename="figures-PDM/Shap-Summ-Soc-PDM.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.lines.soc.fast,filename="figures-PDM/Shap-lines-Soc.Fast-PDM.bmp",width=7,height=7,units = c("in"))
...
ggsave(plot=plt.shap.summ.soc.fast,filename="figures-PDM/Shap-Summ-Soc.Fast-PDM.bmp",width=7,height=7,units = c("in"))
...
renamed_labels.2["Seg.Neut.perc"] <- "% PMNs"
renamed_labels.2["Lymph.Perc"] <- "% Lymphs"
...
ggsave(plot=plt.shap.summ.soc.fast.10 + theme(legend.position ="none",axis.text=element_text(size=8),
            ...
            plot.title = element_text(hjust = 0.5,size=8)) ,filename="figures-PDM/Shap-Summ-Soc.Fast.10-PDM.bmp",width=1.8,height=0.9,units = c("in"))
...
ggsave(plot=g,filename="figures-PDM/Shap.Full.AB-PDM.bmp",width = 7,height = 7, units = "in")
...
ggsave(plot=plt.cohort.boxplots,file='figures-PDM/Cohort-Boxplots-PDM.bmp')
```
...
plt.cohorts %>% ggsave(file="figures-PDM/Cohort-6-PDM.bmp",height=10,width=7,units=c("in")); plot(plt.cohorts)
...
plt.egfr.only %>% ggsave(file="figures-PDM/A1c-2hG-eGFR-only-PDM.bmp",height=10,width=7,units=c("in"))
...
df.importance  %>% left_join(lookup_df,by=c("Feature"="Y")) %>% dplyr::rename(Type=X) %>% mutate(Feature=factor(Feature,levels=var.features.2)) %>% arrange(Feature)  %>% mutate(across(where(is.numeric), round, 5)) %>% write.csv(file="~/Documents/NHANES15-16/Model-Importance_2025.02.25-PDM.csv",row.names = FALSE)
```
...
ggsave(plt.calibration.quantile,file="figures-PDM/Quantile-calibration-Pred.SOC.Fasting-PDM.bmp")
...
ggsave(file='figures-PDM/Calibration_cohorts-PDM.bmp',plot=plt.calibration.cohorts)
...
x0.pphg <- as.factor(office_test$two >= 140)
...
#pROC::roc(predictor=office_test$fpg,response=(x0.igt==TRUE & office_test$fpg>=100 & office_test$fpg < 100)) #%>% pROC::coords(.,100,input="threshold")
...
pROC::roc(predictor=office_test$fpg,response=(x0.igt==TRUE |( office_test$fpg>=100 & office_test$fpg < 100) ))  %>% pROC::coords(.,x="best",ret=c("threshold","specificity","sens"))

pROC::roc(predictor=office_test$fpg,response=(x0.pphg==TRUE | office_test$fpg>=100)) %>% pROC::coords(.,x="best",ret=c("threshold","specificity","sens"))

pROC::roc(predictor=office_test$fpg,response=(x0.pphg==TRUE | office_test$fpg>=100 | office_test$a1c >=5.7)) %>% pROC::coords(.,x=100,input="threshold",ret=c("threshold","specificity","sens"))
...
c("Type"="Pre-DM","Vars"="FPG.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$fpg,response=(x0.igt)) %>% 
            pROC::coords(.,x=99.9,input="threshold",ret=c("threshold","specificity","sensitivity")) ),
c("Type"="DM","Vars"="FPG.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$fpg,response=(x0.pphg)) %>% 
            pROC::coords(.,x=125.99,input="threshold",ret=c("threshold","specificity","sensitivity")) ),

c("Type"="Pre-DM","Vars"="A1c.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$a1c,response=(x0.igt)) %>% 
            pROC::coords(.,x=5.69,input="threshold",ret=c("threshold","specificity","sensitivity"))),
c("Type"="DM","Vars"="A1c.Fixed","AUROC"=NA,pROC::roc(predictor=office_test$a1c,response=(x0.pphg)) %>% 
            pROC::coords(.,x=6.49,input="threshold",ret=c("threshold","specificity","sensitivity")))
...
```{r Making Stats 2.2}
f.pROC.cutoff <- function(predictor.x,response.x) {pROC::coords(pROC::roc(predictor=predictor.x,
                                                                          response=response.x),
                                                                "best",
                                                                ret=c("threshold","specificity","sens")) }
...
x0.pphg <- as.factor(office_test$two >= 140 | office_test$a1c >=5.7 | office_test$fpg >=100)


f.pROC.cutoff(office_test$a1c,x0.pphg)
...
df.stats.2 <- rbind(
  ...
  filter(Vars!="FPG.Fixed" & Vars!="A1c.Fixed") ### These are inaccurate so exclude them
...
x0.pphg.bool <- (office_test$two >= 140 | office_test$a1c >=5.7 | office_test$fpg >=100)


# A1c
stringent.threshold.dm <- list(
     TP=sum((office_test$a1c >= 5.7) ), # TPs
     TN=sum((office_test$a1c < 5.7)==TRUE & !x0.pphg.bool), # TNs
     FN=sum((office_test$a1c < 5.7)==TRUE & x0.pphg.bool), # FN
     FP=0 #FPs
)
...
stringent.threshold.dm.fpg <- list(
     TP=sum(office_test$fpg >= 100 ), # TPs
     TN=sum(office_test$fpg < 100 & !x0.pphg.bool ), # TNs
     FN=sum(office_test$fpg < 100 & x0.pphg.bool ), # FN
     FP=0 #FPs
)
...
) %>% mutate(FPR=round(FPR,2),TPR=round(TPR,2))
```
...
```{r}
roc_results.3 <- roc_results.2 %>% mutate(Specificity=1-FPR) %>% rename(Sensitivity=TPR) %>% select(-c(FPR)) %>% group_by(Type,Type2,Dx,Cutoff) %>% pivot_longer(cols = c("Specificity","Sensitivity"),values_to = "Value",names_to = "Stat" ) %>% ungroup() %>% relocate(Stat,Dx,Type2,Value) %>% select(Stat,Dx,Type2,Value) %>% rename(Type=Dx,Vars=Type2)
...
```{r}
df.stats.3 <- rbind(df.stats.2,roc_results.3)
```
...
df.stats.4<- df.stats.3 %>% pivot_wider(names_from = "Vars",values_from = "Value")  %>% arrange(rev(Type),rev(Stat)) %>% relocate(Type,Stat,A1c.Fixed,A1c,FPG.Fixed,FPG,`FPG-A1c`,FIB4,NonFasting,Min,Full) %>% filter(Type=="DM") %>%  select(Stat,FPG.Fixed) %>% pivot_wider(names_from = Stat,values_from = FPG.Fixed) %>% mutate(Type2="FPG.Fixed")
...
roc_results %>% filter(Dx=="DM") %>% filter(Type=="FPG.Fixed" | Type=="FPG" | Type=="A1c/FPG+") %>% 
  ...
  ggplot() + 
  geom_line(aes(x=FPR,y=TPR,color=Type,linetype=Type)) +
  geom_point(data=df.stats.4,aes(x=1-Specificity,y=Sensitivity,color=Type2))+
  labs(color="Model",linetype="Model",shape="Model") + 
  xlab('False Positive Rate') + 
  ylab('True Positive Rate')  + 
  theme(plot.title = element_text(hjust = 0.5))  + 
...
```{r}
roc_results %>% filter(Dx=="DM" & Type2=="FPG.Fixed")

roc_results.2 %>% filter(Dx=="DM" & Type2=="FPG.Fixed")
```
...
plt.dca.dm<-plot(plot.dca.eval.dm + 
       coord_cartesian(xlim=c(-0.001,0.31),ylim = c(-.001,0.6)) + 
...
plot.dca.eval.dm.few.fast<-plot(plot.dca.eval.dm.few.fast + 
       coord_cartesian(xlim=c(-0.001,0.31),ylim = c(-.001,.6)) + 
       xlab("Risk needed to warrant extra testing")) +
  ylab("Net Benefit")+
  scale_x_continuous(breaks=c(0.0,0.1,0.2,0.3),labels=c("0%","10%","20%","30%")) %>% 
  suppressWarnings()
...
grid.arrange(nrow=4,heights=c(4,1,4,1),
            ...
            arrangeGrob(ncol=2,
             ...
             legend.2) %>% suppressWarnings() %>% ggsave(file="figures-PDM/ROC-DCA-figure-4-PDM-PDM.bmp",height = 7,width = 7,units = c("in"))
...
grid.arrange(nrow=3,heights=c(4,1,1.2),
             ...
             ),top="Diagnosis of Diabetes") %>% suppressWarnings() %>% ggsave(file="figures-PDM/ROC-DCA-figure-AB-PDM.bmp")
...
#              ),top="Diagnosis of Diabetes") %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/ROC-DCA-figure-AB-Few-PDM.bmp")
...
grid.arrange(nrow=3,heights=c(5,1.5,3),
             ...
             ),top="Diagnosis of Pre-Diabetes") %>% suppressWarnings() %>% ggsave(file="figures-PDM/ROC-DCA-figure-AB-Few-PDM.bmp")
