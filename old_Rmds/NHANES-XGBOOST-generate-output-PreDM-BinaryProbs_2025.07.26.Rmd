...
Var.SOC = c(Var.Clinic,Var.Hx,Var.CBC,Var.CMP,'choles',"HepB.sAb","HepB.cAb")
Var.SOC.fast = c(Var.SOC,"fpg")
...
Here we are importing the data from Simar's dataset where office_test, office_pre, and office_train come separately. We will recombine and treat it as df.alt
```{r ALTERNATIVE IMPORT}
load("~/Documents/NHANES-DMDx/NHANES-office-test_2025.07.29-BinaryProbs.Rda")
load("~/Documents/NHANES-DMDx/NHANES-office-pre_2025.07.29-BinaryProbs.Rda")
load("~/Documents/NHANES-DMDx/NHANES-office-train_2025.07.29-BinaryProbs.Rda")




all.equal(names(office_test),names(office_pre))
all.equal(names(office_test %>% select(-Pred.Full)),names(office_train ))


sort(names(office_test))
office_test %>% select(Year) %>% table()
office_pre %>% select(Year) %>% table()
office_train %>% select(Year) %>% table()
```
...
f.is.PreDM <- function(df.fx){
  dm =ifelse( (is.na(df.fx$a1c) & is.na(df.fx$fpg) & is.na(df.fx$two)), NA, 
              ifelse( !is.na(df.fx$a1c) & df.fx$a1c >= 5.7, TRUE, 
                ifelse(!is.na(df.fx$fpg) & df.fx$fpg >= 100,TRUE,
                  ifelse(!is.na(df.fx$two) & df.fx$two >= 140,TRUE,FALSE))))
  return(dm)
}  


office_test$PDM <- f.is.PreDM(office_test)
office_pre$PDM <- f.is.PreDM(office_pre)
office_train$PDM <- f.is.PreDM(office_train)

office_test$dm.told <- office_test$dm.told.x
office_pre$dm.told <- office_pre$dm.told.x
office_train$dm.told <- office_train$dm.told.x

office_test <- office_test %>% select(-c(dm.told.x,dm.told.y))
office_pre <- office_pre %>% select(-c(dm.told.x,dm.told.y))
office_train <- office_train %>% select(-c(dm.told.x,dm.told.y))


df.alt <- rbind(office_train,
                office_test %>% select(-Pred.Full)) %>% select(-DM)
...
#df.small <- df %>% drop_na(a1c,two,sample.ogtt.weight,fpg)
df.small <- df.alt %>% drop_na(a1c,two,sample.ogtt.weight,fpg)


#df.small$DM <- as.factor(df.small$a1c >= 6.5 | df.small$two >= 200 | df.small$fpg >=126)
#df.small$PDM <- ((df.small$a1c >= 5.7 & df.small$a1c < 6.5) | (df.small$two >= 140 & df.small$two < 200) | (df.small$fpg >=100 & df.small$fpg<126))
...
date.string <- "2025.07.31-Binary-PreDM"


# dir.string <-"~/Documents/NHANES15-16/"
# date.string <- "2025.06.26-Binary-ValidTest"
...
f.clean.data.for.xgboost <- function(df.small){
  ...
  label.train = office_train$PDM
  label.test = office_test$PDM
  ...
  office_train = office_train %>% select(-c(SEQN,sample.ogtt.weight,Year,two,PDM)),
  office_test = office_test %>% select(-c(SEQN,sample.ogtt.weight,Year,two,PDM))
 ))
}
...
f.clean.data.for.xgboost.external <- function(df.ext){
  ...
  office_ext <- df.ext %>% select(-DM)
  
  SEQN.ext = office_ext$SEQN

  year.ext = office_ext$Year

  label.ext = office_ext$PDM
  ...
  office_ext = office_ext %>% select(-c(SEQN,sample.ogtt.weight,Year,two,PDM))
 ))
}
...
f.just.prepare.data.external <- function(clean.output,config){
  ...
  return(list(ext_X,dext,test_X,dtest,weight.sample.ext.norm))
}
...
"Anemia.Treatment", "a1c", "dm.told", "Citizenship", "MCHC", 
...
"Anemia.Treatment", "fpg", "a1c", "dm.told", "Citizenship", "MCHC", 
"mcv", "Mean.Cell.Hemoglobin", "rbc", "hg", "wbc", "RBC.Dist.Width", 
"plt", "mpv", "Seg.Neut.perc", "LBXLYPCT", "Monocyte.perc", "eosin.perc", 
"Baso.Perc", "Weight", "Height.Stand", "na", "bun", "cr", "alb", 
"chlor", "alp", "potas", "tprot", "tbili", "bicarb", "cal", "alt", 
...
model_configs <- list(
  ...
  SOC = list(
    mutate_expr = ~ select(.,all_of(Var.SOC.2)),
   pred_col = "Pred.SOC"
  ),
  SOC.Fast = list(
    mutate_expr = ~ select(.,all_of(Var.SOC.Fast.2)),
...
office_train$PDM <- clean.output$label.train
...
office_test$PDM <- clean.output$label.test
...
```{r}
### Clean data - this only needs to be done once
clean.output <- f.clean.data.for.xgboost.external(office_pre %>% select(-Pred.Full))
...
office_ext$PDM <- clean.output$label.ext
...
contr <- predict(xgb_models$Full, prepped$Full[[1]], predcontrib = TRUE)


shap.dat<-xgb.plot.shap(prepped$Full[[1]], contr, model = xgb_models$Full, top_n = 20, n_col = 5)
```


```{r}
shap.dat.2 <- left_join(
shap.dat$shap_contrib %>% data.frame() %>% mutate(row=row_number()) %>% pivot_longer(cols=!row,names_to = "Feature",values_to = "Shap"),
shap.dat$data %>%data.frame() %>% mutate(row=row_number()) %>% pivot_longer(cols=!row,names_to = "Feature",values_to = "Value"),join_by(row,Feature)) %>%
...
features.ordered <-shap.dat.2 %>% filter(row==1) %>% select(Feature) %>% pull()
shap.dat.2$Feature <- factor(shap.dat.2$Feature,levels=features.ordered,ordered = TRUE)



plt.shap.lines <- shap.dat.2 %>% ggplot(aes(x=Value,y=Shap)) + geom_point(size=0.5) + geom_smooth() + facet_wrap(~Feature,scales = "free") + ylab("SHAP Values");plot(plt.shap.lines)
...
plt.shap.summ <- xgb.ggplot.shap.summary(prepped$Full[[1]], contr, model = xgb_models$Full, top_n = 20) + 
  scale_x_discrete("Feature",labels=c( 'fpg' = 'Fasting Plasma Glucose',
                                      'a1c' = 'Hemoglobin A1c',
                                      'tgy' = 'Triglycerides',
...
#ggsave(filename="~/Dropbox/NHANES manuscript/figures/Pre-DMShap-Summary.bmp",plot=plt.shap.summ)
...
shap_values <- shap.values(xgb_model =xgb_models$Full , X_train = prepped$Full[[1]])
# The ranked features by mean |SHAP|
shap_values$mean_shap_score

# To prepare the long-format data:
shap_long <- shap.prep(xgb_model = xgb_models$Full, X_train = prepped$Full[[1]])
# is the same as: using given shap_contrib
#shap_long <- shap.prep(shap_contrib = shap_values$shap_score, X_train = prepped$Full[[1]])
...
dict.20 <- c( 'fpg' = 'Fasting Plasma Glucose',
                                      'a1c' = 'Hemoglobin A1c',
                                      'Age' = 'Age',
                                      'tgy' = 'Triglycerides',
                                      'SBP' = 'Systolic Blood Pressure',
...
features.20 <- dict.20 %>% names() 


shap_long %>% select(variable,mean_value) %>% unique() %>% slice_max(n=20,order_by = abs(mean_value))

shap.plot.summary(shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)))
...
```{r}
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% mutate(quantile=cut_number(rfvalue,10)) %>% 
   mutate(Var2 = str_replace_all(variable,dict.20))
```
...
ord.n <- 5
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% group_by(variable) %>%  reframe(variable=variable,value=value,ord=as.integer(cut_number(rfvalue,ord.n,na.rm=TRUE))) %>% 
  group_by(variable)  %>% 
  mutate(Var2 = factor(str_replace_all(variable,dict.20),levels=rev(dict.20))) %>% 
  ggplot() + 
  geom_point(aes(y=Var2,x=value,color=ord),alpha=0.3) + 
  scale_color_viridis_c(limits=c(0,ord.n),breaks=c(0,ord.n),direction=-1,labels=c('Low','High'),name="Feature") 
