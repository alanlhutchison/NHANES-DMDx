...
#file.xgb_tuned.full <- "~/Documents/NHANES15-16/NHANES-training-XGBOOST_xgb_tuned.29_2024.12.10.Rda"
save(xgb_tuned.full.29,file=file.xgb_tuned.full)
...
office_train.makeX <- makeX(office_train.Base)
dtrain <- xgb.DMatrix(data = makeX(office_train.Base), label = label.train)


office_test.Base <- office_test %>% select(all_of(c(Var.Clinic,Var.Hx,Var.CBC,Var.CMP)))

office_test.makeX <-  makeX(office_test.Base)
...
office_train.MIN <- office_train %>% select(all_of(c(Var.MIN )))

office_train.makeX <- makeX(office_train.MIN)
dtrain <- xgb.DMatrix(data = office_train.makeX, label = label.train)

office_test.MIN <- office_test %>% select(all_of(c(Var.MIN )))

office_test.makeX <-  makeX(office_test.MIN)
office_test.makeX <- f.align_columns(office_train.makeX,office_test.makeX)
...
xgb.model.MIN<- xgboost::xgb.train(params=xgb_tuned.MIN$bestTune %>% flatten(),dtrain,nrounds=2000)
office_test$Pred.MIN <- predict(xgb.model.MIN,dtest)
...
#file.office.train <- '~/Documents/NHANES15-16/NHANES-office-train-layered_2024.11.22.Rda'
save(office_train,file=file.office.train)

### This is a Rda of the office_test data frame 

#file.office.test <- '~/Documents/NHANES15-16/NHANES-office-test-layered_2024.11.22.Rda'
...
#file.list.models <- "~/Documents/NHANES15-16/NHANES-A1c-All-xgb.models"
...
df.stat.2 <- rbind(df.stat.2,f.df.stat(x,"Pred.MIN",z)) 
...
df.stat.manual <- rbind(df.stat.manual,f.cutoff(x,"Pred.MIN",z,140))
...
df.stat.manual <- rbind(df.stat.manual,f.cutoff(x,"Pred.MIN",z,200))
...
f.dm <- function(x){
  if(x >=200){ y="diabetes" }else if (x < 200 & x >=140){y="prediabetes"} else {y="no DM"}
  return(y)
}
...
df %>% drop_na(sample.ogtt.weight,a1c,two) %>% select(a1c,two,sample.ogtt.weight) %>% mutate(diabetes_status = sapply(two,f.dm)) %>% filter(a1c<7) %>% sample_n(size=100000,weight = sample.ogtt.weight,replace=TRUE)  %>% mutate(a1c_group = cut(a1c, seq(3.3, 7, 0.4)) )   %>% 
  ...
  geom_dotplot(binaxis = "y",
               ...
               dotsize = 0.2,
               stackratio = 0.5) +
  scale_fill_manual(values = c("no DM" = "blue", "prediabetes" = "green", "diabetes" = "maroon"))  +
...
df %>% drop_na(sample.ogtt.weight,a1c,two) %>% select(a1c,two,sample.ogtt.weight) %>% mutate(diabetes_status = sapply(two,f.dm)) %>% filter(a1c<10) %>% sample_n(size=100000,weight = sample.ogtt.weight,replace=TRUE)  %>% mutate(a1c_group = cut(a1c, seq(3.3, 8, 0.4)) )   %>% 
  ggplot( aes(x = a1c_group, y = two)) +
  geom_boxplot(alpha=0.5) +   
  geom_dotplot(binaxis = "y",
               stackdir = "center",
               binwidth = 1,
               dotsize = 0.2,
               stackratio = 0.5, aes(color = diabetes_status,fill= diabetes_status)) +
...
office_test[int.resample.test,] %>% 
  ...
  mutate(Var2.PDM = Pred.MIN >= f.get.thresh("Pred.MIN",s)& Pred.MIN < f.get.thresh("Pred.MIN","DM") )%>%
...
df.pdm.confusion.4 <- office_test[int.resample.test,] %>% filter(a1c < 5.7) %>% 
  ...
  mutate(Fasting.PDM = Pred.Fasting >= f.get.thresh("Pred.Fasting",s)& Pred.Fasting < f.get.thresh("Pred.Fasting","DM") )%>%
  mutate(Urine.Var.PDM = Pred.Urine.Var >= f.get.thresh("Pred.Urine.Var",s)& Pred.Urine.Var < f.get.thresh("Pred.Urine.Var","DM") )%>%
  mutate(Min.PDM = Pred.MIN >= f.get.thresh("Pred.MIN",s)& Pred.MIN < f.get.thresh("Pred.MIN","DM") )%>%
  mutate(Full.PDM = Pred.Full >= f.get.thresh("Pred.Full",s)& Pred.Full < f.get.thresh("Pred.Full","DM") )%>% 
  ...
  ungroup() %>% pivot_longer(-a1c_group,values_to = "Count") %>%
    ...
    mutate(Model = factor(Model,levels=c("A1c","Base","Fasting","SDOH","UrineVar","Min", "Full"))) %>% mutate(Percentage = Count / resample.size * 100)  %>% mutate(`A1c Group`= if_else(a1c_group=="[3.7,4.1)" | a1c_group=="[4.1,4.5)","[4.0,4.5)",a1c_group)) %>% group_by(`A1c Group`,Stat,Diagnosis,Model) %>% summarise(Count=sum(Count),Percentage=sum(Percentage))
...
df.pdm.confusion.4 %>% rowwise() %>% group_by(`A1c Group`,Diagnosis,Model)
```
...
df.dm.confusion.4<- office_test[int.resample.test,] %>% 
  ...
  mutate(Min.DM = Pred.Var2 >= f.get.thresh("Pred.MIN",s)) %>%
  ...
  summarise(A1c.DM.Sum.TP = sum(A1c.DM & two >=  200),
            ...
            Min.DM.Sum.TP = sum(Min.DM & two >= 200),
            ...
            Min.DM.Sum.FP = sum(Min.DM & two < 200),
            ...
            Min.DM.Sum.FN = sum(!Min.DM & two >= 200),
            ...
            Min.DM.Sum.TN = sum(!Min.DM & two <  200),
...
df.dm.confusion.4 %>%
  ...
  filter(Stat %in% c("Expected","Observed"),Model=="A1c") %>% 
  ggplot(aes(x=`A1c Group`,y=Count,group=Stat)) + geom_col(aes(fill=Stat),position = "dodge") 
```
