...
date.string <- "2026.01.08_2005test"
filter.string <- "all"
...
print('Age > 60')
df.small %>% filter(Age>60,Year>2005) %>% nrow() 
df.small %>% filter(Age>60,Year==2005) %>% nrow() 

print('FIB-4 > 2.67')
df.small %>% mutate(FIB4 = Age/plt*ast/sqrt(alt)) %>% filter(FIB4>2.67,Year>2005) %>% nrow() 
df.small %>% mutate(FIB4 = Age/plt*ast/sqrt(alt)) %>% filter(Year==2005,FIB4>2.67) %>% nrow() 

print('BMI > 30')
df.small %>% mutate(BMI = Weight/(Height.Stand^2)*100*100)  %>%  filter(BMI>30,Year>2005) %>% nrow() 
df.small %>% mutate(BMI = Weight/(Height.Stand^2)*100*100)  %>%  filter(BMI>30,Year>2014) %>% nrow() 

print('SBP > 140')
df.small %>% filter(SBP>140,Year>2005) %>% nrow() 
df.small %>% filter(SBP>140,Year==2005) %>% nrow() 
...
ggsave(plot=plt.shap.summ.soc,filename="figures/2.Shap-Summ-Soc.pdf",width=7,height=7,units = c("in"),useDingbats = TRUE)
ggsave(plot=plt.shap.summ.soc,filename="figures/2.Shap-Summ-Soc.tif",width=7,height=7,units = c("in"),dpi=100)
...
ggsave(plot=plt.shap.summ.soc.fast.10 + theme(legend.position ="none",axis.text=element_text(size=8),
            ...
            plot.title = element_text(hjust = 0.5,size=8)) ,filename="figures/Shap-Summ-Soc.Fast.10.bmp",width=1.8,height=0.9,units = c("in"))
...
plt.shap.summ.soc <- xgb.ggplot.shap.summary(prepped$SOC[[1]], contr.soc, model = xgb_models$SOC, top_n = 20,subsample=0.1) + 
  scale_x_discrete("Feature",labels=renamed_labels)+
  ylab("SHAP value") + labs(color="Feature\nvalue") + ggtitle("A1c+ Model")  + theme.1 + 
  scale_color_viridis_c(limits=c(-10,10),breaks=c(-10,10),direction=-1,labels=c('Low','High')) ;plot(plt.shap.summ.soc)
...
names.stats <- names.stats[! names.stats %in% c('Year','Year.2005')]
...
t2 <- names( df.small %>% select(features,Year) %>% 
                         mutate(Year = ifelse(Year == 2005,"2005-2006,<br>OGTT",ifelse(Year > 2005,"1999-2004","2007-2016,<br>OGTT"))) )
...
df.desc.stats <- rbind(df.mo3  %>% #rename(SBP=sbp,DBP=dbp,Lymph.Perc=LBXLYPCT,Poverty.Ratio=PovertyRatio) %>% 
                       ...
                       df.small %>% select(features,Year) %>% 
                         mutate(Year.Group = ifelse(Year == 2005,"2005-2006,<br>OGTT",ifelse(Year < 2005,"1999-2004","2007-2016,<br>OGTT"))) %>% select(-Year) ) %>% 
...
```{r}
features <- features.top20.soc.fast
df.desc.stats <-df.small %>% select(features,Year) %>% mutate(Year.2005 = ifelse(Year == 2005,"2005-2006","2007-2016")) %>% 
  rename_with(~ str_replace_all(., c(
...
library(htmlTable)
library(Gmisc)
df.desc.stats %>% getDescriptionStatsBy(by=Year.2005, names.stats,
#`Fasting Glucose Plasma&dagger;`=fpg,				
add_total_col = FALSE) %>% 
  addHtmlTableStyle(pos.caption = "bottom") %>% 
  htmlTable(caption  = "Basic descriptive statistics from the NHANES dataset")#,
...
plt.calibration.quantile <- office_test %>% mutate(bin=cut_number(Pred.SOC.Fast, n = 10)) %>% group_by(bin) %>% reframe(med=median(Pred.SOC.Fast),freq=sum(DM==TRUE)/sum(!is.na(DM))) %>% ggplot() + geom_point(aes(x=med,y=freq)) + geom_line(data=data.frame(x=c(0,1),y=c(0,1)),aes(x=x,y=y),linetype="dashed") +  geom_rug(data=office_test,aes(x=Pred.SOC.Fast,y=as.integer(DM)),alpha=0.6) + xlab("Median of Probaility Quantile") + ylab("Frequency of Diabetes Diagnosis") + xlim(0,1) + ylim(0,1) + ggtitle("Frequency of Diagnosis by Quantile for A1c/FPG+ Model") + theme.1; plot(plt.calibration.quantile)

ggsave(plt.calibration.quantile,file="figures/Quantile-calibration-Pred.A1cFPG+.bmp")
...
dca_plot <- function(data,outcome, prefix, title) {
  ...
  dca(formula, data = df.dca.eval, 
    ...
    scale_linetype_manual(values=c("A1c"="longdash","Treat All"="dotted","A1c+"="longdash","A1c.Fixed"="dotted"))
}
...
grid.arrange(nrow=4,heights=c(4,1,4,1),
            ...
            arrangeGrob(ncol=2,
             ...
             legend.2) %>% suppressWarnings() %>% ggsave(file="figures/ROC-DCA-figure-4.bmp",height = 7,width = 7,units = c("in"))
...
grid.arrange(nrow=4,heights=c(4,1,4,1),
            ...
            arrangeGrob(ncol=2,
                         plt.2 + theme(legend.position="none") + theme.1 + labs(tag="C"),
                          plot.dca.eval.dm.few.fast + theme(legend.position="none")+ theme.1 + labs(tag="D")),
             legend.2) %>% suppressWarnings() %>% ggsave(file="figures/3.ROC-DCA-figure-4.pdf",height = 7,width = 7,units = c("in"))
...
grid.arrange(nrow=3,heights=c(5,1.5,3),
             ...
             ),top="Diagnosis of Diabetes") %>% suppressWarnings() %>% ggsave(file="figures/ROC-DCA-figure-AB-Few.bmp")
