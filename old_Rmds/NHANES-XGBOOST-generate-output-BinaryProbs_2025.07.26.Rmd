...
office_test$DM <- f.is.DM(office_test)
office_pre$DM <- f.is.DM(office_pre)
office_train$DM <- f.is.DM(office_train)



# Shouldn't really have DM Told in the model at all
...
df.small <- df %>% drop_na(a1c,two,sample.ogtt.weight,fpg) %>% select(-EduLevel)
df.small$DM <- f.is.DM(df.small)

df.small.2 <- df.alt %>% drop_na(a1c,two,sample.ogtt.weight,fpg)
df.small.2 <- df.small.2 %>% rename(SBP=sbp,DBP=dbp,
                                  Poverty.Ratio = PovertyRatio,
                                  RaceEth=Race,
                                  Num.People.Household=Ppl.In.Household,
                                  EduLevel=EducationStatus,
                                Lymph.Perc=LBXLYPCT,
                                Ever.Told.Overweight=MCQ080,
                                CitizenStatus=Citizenship) %>% select(-DM)



office_pre<- office_pre  %>% rename(SBP=sbp,DBP=dbp,
                                  Poverty.Ratio = PovertyRatio,
                                  RaceEth=Race,
                                  Num.People.Household=Ppl.In.Household,
                                  EduLevel=EducationStatus,                                  
                                Lymph.Perc=LBXLYPCT,
                                Ever.Told.Overweight=MCQ080,
                                CitizenStatus=Citizenship) %>% select(-c("DM","Pred.Full","EduLevel","Anemia.Treatment"))
...
date.string <- "2025.09.18-Binary_test"
