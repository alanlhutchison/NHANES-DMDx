...
```{r}
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% ggplot() + 
  geom_point(aes(x=rfvalue,y=stdfvalue,color=variable)) 
```

```{r}
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% group_by(variable) %>% 
  summarise(variable=variable,value=value,z=mosaic::zscore(rfvalue)) %>% ggplot() + geom_histogram(aes(x=z,color=variable))
```
...
ord.n <- 5
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% group_by(variable) %>%  reframe(variable=variable,value=value,quantile=cut_number(rfvalue,ord.n,na.rm=TRUE)) %>% 
  group_by(variable) %>% reframe(variable=variable,value=value,ord=as.integer(quantile)) %>% 
...
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% group_by(variable) %>%  reframe(variable=variable,value=value,quantile=cut_number(rfvalue,ord.n,na.rm=TRUE)) %>% 
  group_by(variable) %>% reframe(variable=variable,value=value,ord=as.integer(quantile)) %>% ggplot() + geom_histogram(aes(x=ord))
