...
plt.calibration<-office_test   %>% 
  ...
  geom_line(data=data.frame(x=c(40,500),y=c(40,500)),aes(x=x,y=y),linetype="dashed")+ xlim(c(30,550)) + ylim(c(30,550)) + scale_color_viridis(name='Hemoglobin\nA1c (%)',direction=-1) + 
...
plt.calibration.cohort.age <-ot %>% 
  ...
  geom_point(data=ot %>% filter( a1c<6.5),aes(x=Pred.Min,y=two,color=a1c),alpha=0.7) +  
  annotate("text",x=400,y=120,label=paste0("R^2 == ",round(cor.1,2)),parse=TRUE) + 
  #geom_point(data=office_test %>% filter(a1c < 4.5 ),aes(x=Pred.Min,y=two,color=a1c),color='yellow',alpha=0.7) +  
  geom_line(data=data.frame(x=c(30,550),y=c(30,550)),aes(x=x,y=y),linetype="dashed")+ xlim(c(30,550)) + ylim(c(30,550)) + scale_color_viridis(name='Hemoglobin\nA1c (%)',direction=-1,limits=c(4.0,6.5))+ ggtitle("Age ≥ 60 years") +
  xlab("Predicted 2-hour Glucose") + ylab("Measured 2-hour Glucose");plot(plt.calibration.cohort.age)

ot <- office_test %>% mutate(FIB4=ast/sqrt(alt)*Age/plt) %>% filter(FIB4 > 2.67)
cor.1 <- cor(ot$two,ot$Pred.Min)^2
plt.calibration.cohort.fib4 <-ot %>% 
  ...
  annotate("text",x=400,y=120,label=paste0("R^2 == ",round(cor.1,2)),parse=TRUE) + 
  #geom_point(data=office_test %>% filter(a1c < 4.5 ),aes(x=Pred.Min,y=two,color=a1c),color='yellow',alpha=0.7) +  
  geom_line(data=data.frame(x=c(30,550),y=c(30,550)),aes(x=x,y=y),linetype="dashed")+ xlim(c(30,550)) + ylim(c(30,550)) + scale_color_viridis(name='Hemoglobin\nA1c (%)',direction=-1,limits=c(4.0,6.5)) + ggtitle("FIB-4 > 2.67") +
  xlab("Predicted 2-hour Glucose") + ylab("Measured 2-hour Glucose");plot(plt.calibration.cohort.fib4)


ot <- office_test %>%  filter(Weight^2/Height.Stand >= 30)
cor.1 <- cor(ot$two,ot$Pred.Min)^2
plt.calibration.cohort.bmi <-ot %>% 
  ...
  annotate("text",x=400,y=120,label=paste0("R^2 == ",round(cor.1,2)),parse=TRUE) + 
  #geom_point(data=office_test %>% filter(a1c < 4.5 ),aes(x=Pred.Min,y=two,color=a1c),color='yellow',alpha=0.7) +  
  geom_line(data=data.frame(x=c(30,550),y=c(30,550)),aes(x=x,y=y),linetype="dashed")+ xlim(c(30,550)) + ylim(c(30,550)) + scale_color_viridis(name='Hemoglobin\nA1c (%)',direction=-1,limits=c(4.0,6.5)) + ggtitle(expression("BMI ≥ 30 kg/m^2")) +
  xlab("Predicted 2-hour Glucose") + ylab("Measured 2-hour Glucose");plot(plt.calibration.cohort.bmi)


ot <- office_test %>% mutate(.,eGFR = 142 *
                       pmin(cr/ifelse(Gender=="Male",0.9,0.7),1,na.rm=TRUE ) ^ifelse(Gender=="Male",-0.302,-0.241) *
                       pmax(cr/ifelse(Gender=="Male",0.9,0.7),1,na.rm=TRUE ) ^-1.20 *
                       0.9938 ^ Age *
...
plt.calibration.cohort.egfr <-ot %>% 
  ...
  annotate("text",x=400,y=120,label=paste0("R^2 == ",round(cor.1,2)),parse=TRUE) + 
  #geom_point(data=office_test %>% filter(a1c < 4.5 ),aes(x=Pred.Min,y=two,color=a1c),color='yellow',alpha=0.7) +  
  geom_line(data=data.frame(x=c(30,550),y=c(30,550)),aes(x=x,y=y),linetype="dashed")+ xlim(c(30,550)) + ylim(c(30,550)) + scale_color_viridis(name='Hemoglobin\nA1c (%)',direction=-1,limits=c(4.0,6.5)) + ggtitle("Estimated GFR < 60 mL/min/1.73 m²") + 
  xlab("Predicted 2-hour Glucose") + ylab("Measured 2-hour Glucose");plot(plt.calibration.cohort.egfr )


ot <- office_test  %>% filter(SBP > 140) 
cor.1 <- cor(ot$two,ot$Pred.Min)^2
plt.calibration.cohort.sbp <-ot %>% 
  ...
  annotate("text",x=400,y=120,label=paste0("R^2 == ",round(cor.1,2)),parse=TRUE) + 
  #geom_point(data=office_test %>% filter(a1c < 4.5 ),aes(x=Pred.Min,y=two,color=a1c),color='yellow',alpha=0.7) +  
  geom_line(data=data.frame(x=c(30,550),y=c(30,550)),aes(x=x,y=y),linetype="dashed")+ xlim(c(30,550)) + ylim(c(30,550)) + scale_color_viridis(name='Hemoglobin\nA1c (%)',direction=-1,limits=c(4.0,6.5)) + ggtitle("Systolic BP > 140 mmHg") + 
  xlab("Predicted 2-hour Glucose") + ylab("Measured 2-hour Glucose");plot(plt.calibration.cohort.sbp )

label.1 <- cowplot::get_legend(plt.calibration.cohort.age)
...
df.stats$Type <- c(rep("Pre-DM",6),rep("DM",6))
...
plt.roc.results.few <- roc_results %>% filter(Type=="FPG" | Type=="A1c" | Type =="NonFasting" | Type=="Min")  %>% ggplot() + 
  ...
  geom_line(data=data.frame(x=c(0,1),y=c(0,1)),aes(x=x,y=y),linetype="dashed")  + 
  scale_color_manual(values=c("A1c"="darkgreen","FPG"="green","FPG-A1c"="cyan","NonFasting"="magenta","FIB4"="purple","Full"="orange","Min"="red"),guide="none") +
  scale_linetype_discrete(labels=c("Diabetes","Pre-Diabetes")); print(plt.roc.results.few) 
...
df.stats.table <- df.stats %>% filter(Stat=="Threshold" | Stat=="Specificity" | Stat=="Sensitivity" | Stat=="AUROC") %>% relocate(Type,Stat,A1c,FPG,`FPG-A1c`,FIB4,NonFasting,Min,Full)

df.stats.grob.Full <- df.stats.table %>% tableGrob(theme=mytheme)

df.stats.table.Few <- df.stats %>% filter(Stat=="Threshold" | Stat=="Specificity" | Stat=="Sensitivity" | Stat=="AUROC") %>% relocate(Type,Stat,A1c,FPG,`FPG-A1c`,FIB4,NonFasting,Min,Full) %>% select(Type,Stat,A1c,FPG,NonFasting,Min) 

df.stats.grob.Few <- df.stats.table.Few %>% tableGrob(theme=mytheme)

plot(df.stats.grob.Few)
...
plot.dca.eval.pdm <- dca_plot(df.dca.eval,"PreDM", "pm.predm", "Pre-Diabetes")
plot.dca.eval.dm <- dca_plot(df.dca.eval,"DM", "pm.dm", "Diabetes")
...
theme.table <- ttheme_default( core= list(bg_params = list(fill=c("grey95","grey90","grey95","grey90")), ###
                               ...
                               colhead = list( fg_params = list(cex = 0.5)),
                                 core = list(padding = unit(c(1, 1), "mm")),      # Reduce padding inside cells
                                colhead = list(padding = unit(c(1, 1), "mm")),  # Reduce padding in column headers
                                rowhead = list(padding = unit(c(1, 1), "mm"))   # Reduce padding in row headers
...
names(df.stats.table.2)[5] <- "FPG-\nA1c"
names(df.stats.table.2)[7] <- "Non\nFasting"
...
grid.arrange(nrow=3,heights=c(3,3,1),
            ...
            legend.2 ) %>% suppressWarnings() %>% ggsave(file="~/Dropbox/NHANES manuscript/figures/ROC-DCA-figure-Full.bmp")
...
names(df.stats.table.Few.2)[5] <- "Non\nFasting"
...
tG$widths <- unit(rep(1/ncol(tG), ncol(tG)), "npc")
tG <- arrangeGrob(title,tG,heights=c(0.01,1),vp = viewport(y = 0.6, height = 0.6,x=0.48))
