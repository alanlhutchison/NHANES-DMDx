...
load("~/Documents/NHANES15-16/NHANES-office-test-layered_all_2025.09.02-Binary.Rda") # office_test
...
```{r}
names(df.mort.noogtt)[!names(df.mort.noogtt) %in% names(office_train)]
names(office_train)[!names(office_train) %in% names(df.mort.noogtt)]
...
pre.features <- names(office_ready)
...
save(office_noogtt,file="~/Documents/NHANES15-16/NHANES-office-2005-2016-noOGTT-layered_all_2025.08.29-Binary.Rda")
```
...
df.mo3 <- rbind(office_pre_final %>% select(-c(ucod_lead,EducationStatus)),
                df.mort3 %>% select(-c("seqn","eligstat","hyperten","permth_int")))
### COMBINE COMBINE
df.mo3 %>% mutate(NAN = !is.na(Pred.SOC.Fast)) %>% select(NAN,Year) %>% table()
save(df.mo3,file="~/Documents/NHANES15-16/NHANES-df.mo3_noogtt.Rda")
