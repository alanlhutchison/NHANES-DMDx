...
```{r}
# load(paste0("~/Documents/NHANES/NHANES-office-pre-full_2025.07.14-BinaryProbs.Rda"))
# load(paste0("~/Documents/NHANES/NHANES-office-train-full_2025.07.14-BinaryProbs.Rda"))
# load(paste0("~/Documents/NHANES/NHANES-office-test-full_2025.07.14-BinaryProbs.Rda"))

#load(paste0("~/Documents/NHANES-DMDx/NHANES-office-pre-full_2025.07.14-BinaryProbs.Rda"))
#load(paste0("~/Documents/NHANES-DMDx/NHANES-office-train-full_2025.07.14-BinaryProbs.Rda"))
#load(paste0("~/Documents/NHANES-DMDx/NHANES-office-test-full_2025.07.14-BinaryProbs.Rda"))



# load("~/Documents/NHANES15-16/NHANES-office-ext-layered_all_2025.07.29-Binary.Rda")
# load("~/Documents/NHANES15-16/NHANES-office-train-layered_all_2025.07.29-Binary.Rda")
# load("~/Documents/NHANES15-16/NHANES-office-test-layered_all_2025.07.29-Binary.Rda")

load("~/Documents/NHANES15-16/NHANES-office-ext-layered_all_2025.08.27-Binary.Rda") # office_ext
load("~/Documents/NHANES15-16/NHANES-office-train-layered_all_2025.08.27-Binary.Rda") # office_train
load("~/Documents/NHANES15-16/NHANES-office-test-layered_all_2025.08.27-Binary.Rda") # office_test


#load("~/Documents/NHANES15-16/NHANES-office-ext-layered_all_2025.07.31-Binary-PreDM.Rda")
#load("~/Documents/NHANES15-16/NHANES-office-train-layered_all_2025.07.31-Binary-PreDM.Rda")
#load("~/Documents/NHANES15-16/NHANES-office-test-layered_all_2025.07.31-Binary-PreDM.Rda")
...
mortality_clean.2005 <- mort.table %>%
  ...
  select(SEQN, mortstat, permth_exm,ucod_leading,diabetes)
...
office_test_final <- office_test %>%
  mutate(SEQN = sprintf("%05d", as.integer(str_trim(as.character(SEQN))))) %>%
  left_join(mortality_clean, by = "SEQN")
...
mstat <- maxstat.test(Surv(permth_exm, as.integer(mortstat)) ~ Pred.SOC.Fast,
                      data = office_pre_final %>% filter((!is.na(a1c) & a1c < 6.5)  | (!is.na(fpg) & fpg < 126) ) %>% filter(!is.na(mortstat)),
                      smethod = "LogRank")
...
survival_data_pre <- prepare_survival_data(office_pre_final)
...
ggplot(df_combined, aes(time, surv, color = group, linetype = source)) +
  ...
  labs(title = "KM Curve – 1999-2004 Mortality Data",
       x = "Time (months)",
...
# Correct call:
plt.km.99.04.pred<-ggsurvplot_df(
  ...
  geom_segment(data=df.seg.1,aes(x=x1,xend=x2,y=y1,yend=y2),size=1) + coord_cartesian(xlim = c(0,280)) +
  ...
  annotate("text",x=df.seg.3$x1,y=1.03*max(df.seg.3$y1,df.seg.3$y2),label="∆",size=5) ;plot(plt.km.99.04.pred)
...
t1 <- survival_data_pre.weighted %>% select(actual_pred_group,ucod_lead) %>% table() ; print(t1)
t1[1,] / sum(t1[1,])
t1[2,] / sum(t1[2,])
t1[3,] / sum(t1[3,])
...
data$DM1 <- as.integer(data$ucod_lead=="007")
...
df.mort2 <- df.master %>% 
  ...
  rename(SEQN=SEQN,Age=RIDAGEYR,Gender=RIAGENDR,# age
              ...
              ## Vital Signs
              pulse = BPXPLS, reg.iregg = BPXPULS, sbp = SBP, dbp = DBP, 
              ## Weight Survey
...
```{r}
df.mort.noogtt <- full_join(df.mort2 %>% mutate(SEQN=as.character(SEQN)),mort.table %>% mutate(SEQN=as.character(seqn)),by=join_by(SEQN==SEQN,Year==Year)) %>% filter(is.na(two))

df.mort2 %>% filter(is.na(two)) %>% drop_na(a1c,fpg,sample.fasting.weight) %>% dim()
df.mort2 %>% filter(is.na(two),is.na(fpg)) %>% drop_na(a1c,WTMEC2YR) %>% dim()

df.mort2 %>% filter(is.na(two),is.na(a1c),is.na(fpg))
...
file.list.models <- "~/Documents/NHANES15-16/NHANES-All-xgb.models_all_2025.08.27-Binary.Rda"
...
office_ready <- df.mort.noogtt[,names(df.mort.noogtt)[names(df.mort.noogtt) %in% names(office_train)]]
...
save(office_ext,file="~/Documents/NHANES15-16/NHANES-office-2005-2016-noOGTT-layered_all_2025.08.29-Binary.Rda")
```


```{r}
df.mort3 <- full_join(office_ext %>% mutate(SEQN=as.character(SEQN)),
                      mort.table %>% mutate(SEQN=as.character(seqn)),by=join_by(SEQN==SEQN,Year==Year)) %>% filter(is.na(two))
...
# Find the cut-point in 'Pred.SOC.Fast' that maximally separates the groups
mstat <- maxstat.test(Surv(permth_exm, as.integer(mortstat)) ~ Pred.SOC.Fast,
                      data = df.mort3 %>% filter((!is.na(a1c) & a1c < 6.5)  | (!is.na(fpg) & fpg < 126) ) %>% filter(!is.na(mortstat)),
                      smethod = "LogRank")
...
df.mo3 <- rbind(office_pre_final %>% select(-c("dm.told","ucod_lead")),
                df.mort3 %>% select(-c("seqn","eligstat","hyperten","permth_int")))
### COMBINE COMBINE
...
ggsave(plot = g1,filename = "~/Dropbox/NHANES manuscript/figures/Surv-DM-cause.png",width=7,height=7,units="in")
