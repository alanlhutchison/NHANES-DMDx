...
file.list.models <- "~/Documents/NHANES15-16/NHANES-All-xgb.models_all_2025.09.02-Binary.Rda"
load(file.list.models) # loads xgb_models


load("~/Documents/NHANES15-16/NHANES-office-train-layered_all_2025.09.02-Binary.Rda")
load("~/Documents/NHANES15-16/NHANES-office-ext-layered_all_2025.09.02-Binary.Rda") # office_ext
...
pre.features <- pre.features[!pre.features %in% c("two","SEQN","Year","sample.ogtt.weight","DM","Pred.SOC.Fast")]
...
save(office_ext,file="~/Documents/NHANES15-16/NHANES-office-2005-2016-noOGTT-layered_all_2025.09.18-Binary.Rda")
```
...
c1 <- names(office_pre_final %>% select(-c("ucod_lead")))
c2 <- names(df.mort3 %>% select(-c("seqn","eligstat","hyperten","permth_int")))
c1[!c1 %in% c2]
c2.remove<-c2[!c2 %in% c1]

df.mo3 <- rbind(office_pre_final %>% select(-c("ucod_lead")),
                df.mort3 %>% select(-c("seqn","eligstat","hyperten","permth_int",c2.remove)))
### COMBINE COMBINE
...
```{r Data - Leading Cause of Death 2}
...
```{r Data All - Diabetes Contributing Factor 2}
# Create the survey design object using your weights column
survey_design <-svydesign(strata=~SDMVSTRA, id=~SDMVPSU, weights=~WTMEC2YR,
                        nest=TRUE, data=data)
...
```{r Data A1c & FPG - Leading Cause of Death 2}
...
```{r Data A1c & FPG - Diabetes Contributing Factor 2}
# Create the survey design object using your weights column
survey_design <-svydesign(strata=~SDMVSTRA, id=~SDMVPSU, weights=~WTMEC2YR,
                        nest=TRUE, data=data)
