---
title: "NHANES-XGBOOST-Predict-Diff"
...
```{r A2DIFF PREDICTOR SET UP}

f.a1c.to.2h <- function(x)  return(75*(x-5.7) + 140)

df.small <- df %>% drop_na(a1c,two,sample.ogtt.weight) %>% filter(a1c<6.5) %>% filter(a1c>=5)
df.small$a2diff <-df.small$two - f.a1c.to.2h(df.small$a1c)
...
label.train <- office_train$a2diff
label.test <- office_test$a2diff
weight.sample.train <- office_train$sample.ogtt.weight
weight.sample.test <- office_test$sample.ogtt.weight
office_train <- office_train %>% select(-c(sample.ogtt.weight,Year,two,a2diff))
office_test <- office_test %>% select(-c(sample.ogtt.weight,Year,two,a2diff))
...
```{r BINARY DM FAILURE PREDICTOR SET UP}

#f.a1c.to.2h <- function(x)  return(75*(x-5.7) + 140)

df.small <- df %>% drop_na(a1c,two,sample.ogtt.weight) %>% filter(a1c<6.5) 
df.small$missed.DM <- as.factor(df.small$two >= 200 & df.small$a1c<6.5)
...
year.train <- office_train$Year
year.test <- office_test$Year
label.train <- office_train$missed.DM
label.test <- office_test$missed.DM
weight.sample.train <- office_train$sample.ogtt.weight
weight.sample.test <- office_test$sample.ogtt.weight
office_train <- office_train %>% select(-c(sample.ogtt.weight,Year,two,missed.DM))
office_test <- office_test %>% select(-c(sample.ogtt.weight,Year,two,missed.DM))
...
```{r XGBOOT FULL - Missed.DM predictor}
...
#Use caret to tune
xgb_tuned.full.missed.DM <- caret::train(
  method = "xgbTree",
...
```{r SAVE Missed.DM predictor}
## creates xgb_tuned.25
save(xgb_tuned.full.29,file="~/NHANES-DMDx/NHANES-training-XGBOOST_xgb_tuned_missed.DM.2024.11.02.Rda")
...
```{r GET Missed.DM predictor OUTPUT}

office_train.makeX <- makeX(office_train)
office_train.makeX <- office_train.makeX[,order(colnames(office_train.makeX))]
...
office_test.makeX<-office_test.makeX[,order(colnames(office_test.makeX))]
...
office_test$Prob.Missed.DM.Full <- predict(xgb.model.full,dtest)


office_test %>% mutate(ILS = inv_logit_scaled(office_test$Pred.Full)) %>% ggplot()+ geom_point(aes(x=a1c,y=ILS)) + facet_wrap(~cut(Age,5))

office_test %>% mutate(ILS = inv_logit_scaled(office_test$Pred.Full)) %>% ggplot()+ geom_point(aes(x=a1c,y=ILS)) + facet_wrap(~cut(fpg,5))

office_test %>% mutate(ILS = inv_logit_scaled(office_test$Pred.Full)) %>% 
  mutate(BMI = Weight / Height.Stand/ Height.Stand*100*100) %>% ggplot()+ geom_point(aes(x=a1c,y=ILS)) + facet_wrap(~cut(BMI,8))
...
save(office_test,file='~/NHANES-DMDx/NHANES-office-test-layered_diff_2024.11.02.Rda')
...
var.vector <- c(rep("Clinic",length(Var.Clinic.2)),
                rep("History",length(Var.Hx.2)-2), ### excluding Ever.Told.OverweightRefused,dm.told.ever$Refused
...
plot.dca.eval.pdm <- dca(PreDM ~ pm.pdm.a1c + pm.pdm.x1 + pm.pdm.x2 + pm.pdm.x3 + pm.pdm.x4 + pm.pdm.x5 +  pm.pdm.x6 ,
    data = df.dca.eval,
...
plot.dca.eval.dm <- dca(DM ~ pm.dm.a1c + pm.dm.x1 + pm.dm.x2 + pm.dm.x3 + pm.dm.x4 + pm.dm.x5 + pm.dm.x6 ,
    data = df.dca.eval,
    thresholds = seq(0, 0.5, 0.05),
    label = list(  
  pm.dm.a1c = "A1c",
...
grid.arrange(nrow=2,plot.dca.eval.pdm,plot.dca.eval.dm) 
