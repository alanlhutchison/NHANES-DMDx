...
date.string <- "2025.05.12-FPG"

Var.Fasting.No.FPG <- c("iron", "tgy", "insulin", "Fasting.Time")
...
MinModel = list(
  subset_expr = ~  select(.,fpg, Age, a1c, insulin, Height.Stand, tgy, SBP, Waist.Circ, choles, u.alb, cr, u.cr, DBP, Poverty.Ratio, Seg.Neut.perc, pulse, iron, Arm.Circ, plt, ggt,Year,sample.ogtt.weight,two),
  filter.string = "MinModel"),

NonFastingModel = list(
  subset_expr = ~  select(.,-Var.Fasting.No.FPG),
  filter.string = "NonFastingModel"
),

NonFastingModelTest = list(
  subset_expr = ~  select(.,-Var.Fasting.No.FPG) %>% sample_n(.,size=1000,replace=FALSE),
  filter.string = "NonFastingModelTest"
)
...
f.clean.data.for.xgboost <- function(df.small){
  ...
  label.train = office_train$fpg
  label.test = office_test$fpg
  ...
  office_train = office_train %>% select(-c(sample.ogtt.weight,Year,two,fpg)),
  office_test = office_test %>% select(-c(sample.ogtt.weight,Year,two,fpg))
 ))
}
...
for (model_name in c("FIB4", "SOC", "SOC.Fast")) {
...
office_train$fpg <- clean.output$label.train
...
office_test$fpg <- clean.output$label.test
...
prep.dat <- prepped$Full[[1]]

contr <- predict(xgb_models$Full, prep.dat, predcontrib = TRUE)

shap.dat<-xgb.plot.shap(prep.dat, contr, model = xgb_models$Full, top_n = 20, n_col = 5)
...
plt.shap.summ <- xgb.ggplot.shap.summary(prep.dat, contr, model = xgb_models$Full, top_n = 20) + 
...
shap_values <- shap.values(xgb_model =xgb_models$Full , X_train = prep.dat)
# The ranked features by mean |SHAP|
shap_values$mean_shap_score

# To prepare the long-format data:
shap_long <- shap.prep(xgb_model = xgb_models$Full, X_train = prep.dat)
# is the same as: using given shap_contrib
#shap_long <- shap.prep(shap_contrib = shap_values$shap_score, X_train = prep.dat)
...
features.20 <- names(shap_values$mean_shap_score)[1:20]

ord.n <- 5
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% 
  group_by(variable) %>%  reframe(variable=variable,value=value,ord=as.integer(cut_number(rfvalue,ord.n,na.rm=TRUE))) %>% 
...
shap_long %>% filter(variable %in% features.20) %>% mutate(variable = droplevels(variable)) %>% 
  group_by(variable) %>%  reframe(variable=variable,value=value,quantile=cut_number(rfvalue,ord.n,na.rm=TRUE)) %>% 
