...
df.small$PDM <- as.factor(df.small$a1c >= 5.7 | df.small$two >= 140 | df.small$fpg >=100)
...
date.string <- "2025.06.05-BinaryProbs-PreDM"
...
f.clean.data.for.xgboost <- function(df.small){
  ...
  office_train = office_train %>% select(-c(sample.ogtt.weight,Year,two,PDM)),
  office_test = office_test %>% select(-c(sample.ogtt.weight,Year,two,PDM))
 ))
}
...
```{r}
office_test %>% filter(fpg<126) %>% ggplot()+ geom_point(aes(x=a1c,y=two,shape=PDM,color=Pred.Full,size=a1c)) + scale_color_viridis_c(direction=-1)

office_test  %>% ggplot()+ geom_point(aes(x=a1c,y=Pred.Full,shape=PDM,color=fpg,size=two)) + scale_color_viridis_c(direction=-1)
office_test  %>% ggplot()+ geom_point(aes(x=fpg,y=Pred.Full,shape=PDM,color=a1c,size=two)) + scale_color_viridis_c(direction=-1)
office_test  %>% ggplot()+ geom_point(aes(x=two,y=Pred.Full,shape=PDM,color=a1c,size=a1c)) + scale_color_viridis_c(direction=-1)
