...
# Function to perform a t-test for a given variable and threshold
perform_t_test <- function(df, variable, threshold, target = "two") {
  df <- df %>% mutate(group = ifelse(.data[[variable]] < threshold, "Below", "Above"))
  
  t_test_result <- t.test(df[[target]] ~ df$group, var.equal = FALSE) # Welch's t-test
  
  tidy(t_test_result) %>%
    mutate(variable = variable, threshold = threshold) %>%
    select(variable, threshold, estimate, statistic, p.value)
}

# Define variables and thresholds
variables <- c("cr", "hg", "BMI", "FIB4", "Age", "SBP")
thresholds <- c(2, 13, 30, 2.67, 60, 140)

# Perform t-tests for all variables and combine results
t_test_results <- map2_dfr(variables, thresholds, ~ perform_t_test(df.boxplot %>% filter(a1c_group=="(4.9,5.7]"), .x, .y))
print(t_test_results)

t_test_results <- map2_dfr(variables, thresholds, ~ perform_t_test(df.boxplot %>% filter(a1c_group=="(5.7,6.5]"), .x, .y))
print(t_test_results)
...
office_test[int.resample.test,] %>% 
  ...
  ungroup() %>% pivot_longer(-a1c_group,values_to = "Count") %>%
    ...
    mutate(Model = factor(Model,levels=c("A1c","Base","Fasting","SDOH","UrineVar", "Full"))) %>% mutate(Percentage = Count / resample.size * 100)  %>% mutate(`A1c Group`= if_else(a1c_group=="(3.7,4.1]" | a1c_group=="(4.1,4.5]","(4.0,4.5]",a1c_group)) %>% group_by(`A1c Group`,Stat,Diagnosis,Model) %>% dplyr::summarise(Count=sum(Count),Percentage=sum(Percentage))
```
...
df.pdm.confusion.4 <- office_test[int.resample.test,] %>% filter(a1c < 5.7) %>% 
  ...
  ungroup() %>% pivot_longer(-a1c_group,values_to = "Count") %>%
    ...
    mutate(Model = factor(Model,levels=c("A1c","Base","Fasting","SDOH","UrineVar","Var2", "Full"))) %>% mutate(Percentage = Count / resample.size * 100)  %>% mutate(`A1c Group`= if_else(a1c_group=="[3.7,4.1)" | a1c_group=="[4.1,4.5)","[4.0,4.5)",a1c_group)) %>% group_by(`A1c Group`,Stat,Diagnosis,Model) %>% dplyr::summarise(Count=sum(Count),Percentage=sum(Percentage))
...
df.dm.confusion.4<- office_test[int.resample.test,] %>% 
  ...
  group_by(a1c_group)  %>% 
  dplyr::summarise(A1c.DM.Sum.TP = sum(A1c.DM & two >=  200),
...
df.pdm.confusion.8<-office_test[int.resample.test,] %>% 
  ...
  ungroup() %>% pivot_longer(-a1c_group,values_to = "Count") %>%
    separate_wider_delim(name,".",names=c("Model","Diagnosis",NA,"Stat")) %>%
    mutate(Model = factor(Model,levels=c("A1c","Base","Fasting","SDOH","UrineVar", "Full"))) %>% mutate(Percentage = Count / resample.size * 100)  %>% mutate(`A1c Group`= if_else(a1c_group=="[3.3,4.1)" | a1c_group=="[4.1,4.9)","[4.0,4.9)",a1c_group)) %>% group_by(`A1c Group`,Stat,Diagnosis,Model) %>% dplyr::summarise(Count=sum(Count),Percentage=sum(Percentage)) 
